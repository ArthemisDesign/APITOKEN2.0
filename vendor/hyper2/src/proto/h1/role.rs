...
/// ПАТЧ (claude-api fingerprint): регистр имён заголовков БАЙТ-В-БАЙТ как у Claude Code (снято с
/// живого claude 2.1.195). Правило Bun/Stainless SDK: `anthropic-*` и `x-app`/`x-client-request-id`
/// идут в lowercase; всё остальное (Accept, Content-Type, User-Agent, X-Stainless-*, X-Claude-Code-*,
/// Authorization, Host, Content-Length, …) — Title-Case по сегментам через '-'. Применяется только на
/// пути preserve_header_case (наш wreq-клиент), поэтому CC-специфика тут безопасна.
fn cc_header_case(dst: &mut Vec<u8>, name: &HeaderName) {
    let s = name.as_str();
