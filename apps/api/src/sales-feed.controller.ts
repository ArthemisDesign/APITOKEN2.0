...
export class SalesFeedController {
  ...
  async usageEvents(@Query("after_id") afterId?: string, @Query("limit") limit?: string) {
    ...
    return {
      items: page.items.map((row) => ({
        ...
        officialNano: row.officialNano?.toString() ?? null,
        chargedNano: row.chargedNano?.toString() ?? null,
        bonusFundedNano: row.bonusFundedNano?.toString() ?? null,
        otherFundedNano: row.otherFundedNano?.toString() ?? null,
        releaseGeneration: row.releaseGeneration?.toString() ?? null,
        releaseDigest: row.releaseDigest ?? null,
  ...
  /**
   * Устанавливает «пол» скидки сейлза для реферала партнёра. Пост-катовер это партнёрская
   * АТРИБУЦИЯ обещанной ставки, а не цена: цена B2C задаётся только активным pricing release
   * (flat 50% + overrides), поэтому scalar/мультипликатор здесь больше не двигается и ответ
