...
function Overview({ account, keys, ledger, open }: { account: AccountView; keys: ApiKeyView[]; ledger: LedgerEntry[]; open(section: Section): void }) {
  ...
  return <section className="panel">
    ...
    <div className="ov-stats"><Stat label="Remaining balance" value={normalizeUsd(account.balanceUsd)} detail={`of ${total} funded`} /><Stat label="Used" value={spent} detail="Official-equivalent usage" /><Stat label="Active keys" value={String(keys.length)} detail="Manage →" onClick={() => open("keys")} /><Stat label="Recent charges" value={String(charged)} detail="View ledger →" onClick={() => open("usage")} /></div>
...
function Usage({ account, ledger, open }: { account: AccountView; ledger: LedgerEntry[]; open(section: Section): void }) {
  return <section className="panel"><PageHeading eyebrow="Activity" title="Usage" subtitle="Authoritative balance movements from the core engine ledger." />
    ...
    <div className="us-cards">
      <div className="card us-route"><div className="us-route-h"><b>API routing</b><span className="pill">API</span></div><span className="dlabel">Available balance</span><div className="us-bal">{normalizeUsd(account.balanceUsd)}</div><span className="p-sub no-margin">{nanoToUsd(account.spentNano)} total official-equivalent usage</span><p className="p-sub compact-top">The same balance covers all Claude models exposed by the core gateway.</p></div>
    ...
    <div className="ov-stats bill4"><Stat label="Requests" value="—" detail="Analytics pending" /><Stat label="Spend (USD)" value={nanoToUsd(account.spentNano)} detail="Official-equivalent spend" /><Stat label="Input tokens" value="—" detail="Analytics pending" /><Stat label="Output tokens" value="—" detail="Analytics pending" /></div>
    <section className="dsec"><div className="trend-head"><h2>Usage trend</h2><div className="trend-ctl"><div className="seg"><button disabled>7d</button><button className="on" disabled>14d</button><button disabled>30d</button></div><div className="seg"><button className="on" disabled>Tokens</button><button disabled>Cost</button></div><button className="btn btn-ghost btn-sm" disabled>Export CSV</button></div></div><div className="chart-card disabled-chart"><div className="chart-legend"><span><i className="legend-blue" />Daily total</span></div><div className="empty-box">Usage analytics are not connected yet.</div></div></section>
    ...
    <section className="dsec"><h2>Token usage</h2><div className="empty-box">No authoritative token totals are available yet.</div></section>
  </section>;
}
