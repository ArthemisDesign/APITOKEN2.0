...
const ABOUT_DESCRIPTION = "apiToken.sale is an independent multi-provider API gateway: the official Anthropic and OpenAI-compatible APIs and models, resold from prepaid balance at a discount, with transparent per-token billing.";

export const metadata: Metadata = {
  ...createPageMetadata({ path: "/about", title: ABOUT_TITLE, description: ABOUT_DESCRIPTION }),
  keywords: ["about apitoken.sale", "claude api gateway", "gpt api gateway", "anthropic api reseller", "openai compatible api provider", "independent api provider"],
};

const aboutUrl = absoluteUrl("/about");

const principles = [
  { h: "The same APIs, transparently", p: "We serve the standard Anthropic Messages API and an OpenAI-compatible API — same endpoints, same model IDs, same responses. Nothing about the protocols is altered; only the price and the way you pay change." },
  ...
  { h: "Accessible payment", p: "Pay by bank card or with cryptocurrency. There is no Anthropic or OpenAI account, waitlist, or supported-billing-country requirement to get started." },
];
