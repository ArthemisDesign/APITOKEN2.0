...
export default function HomePage() {
  return <>
    ...
    <main>
      ...
      <section id="products"><div className="wrap"><div className="prod-grid" data-reveal-stagger><div className="prod"><T k="pc1_tag" as="span" className="tag">Claude API</T><T k="pc1_h" as="h3">All models, one key</T><ul className="prod-list">{models.map((model) => <li key={model}>{model}</li>)}</ul><Link className="btn btn-ghost" href="/models"><T k="pc1_cta">View models</T></Link></div><div className="prod prod-feat"><T k="pc2_tag" as="span" className="tag">Flexible balance</T><div className="amount-example"><span>$</span><b>37</b><T k="amount_example" as="small">example</T></div><T k="pc2_p" as="p">Enter any whole USD amount. No fixed product catalog.</T><Link className="btn btn-primary" href="#pricing"><T k="pc2_cta">See pricing</T></Link></div><div className="prod"><T k="pc3_tag" as="span" className="tag">Free start</T><div className="amt"><T k="p3_now" as="span" className="now">Free</T><span className="was">$2.50</span></div><T k="pc3_p" as="p">Every new account gets $2.50 in test usage. No card required.</T><Link className="btn btn-ghost" href="/register"><T k="start_free">Start free</T></Link></div></div></div></section>
      ...
      <section id="pricing"><div className="wrap"><SectionHead eyebrow="pr_eyebrow" title="pr_h2" lead="pr_lead" /><PricingOverview /><div className="tokens" data-reveal-stagger><T k="tokens_label">Metered per token:</T>{[1,2,3,4,5].map((index) => <T k={`tk${index}`} key={index}>Token type</T>)}</div></div></section>
