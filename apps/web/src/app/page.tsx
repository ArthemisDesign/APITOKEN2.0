...
const faqItems = [
  { question: "q1", answer: "a1", q: "How much does API access cost?", a: "Enter any positive whole USD top-up amount. Each request is converted to official API spend, then the flat 50% discount is applied — the same rate for every account, on every model." },
...
// Сколько официального API получит клиент за пополнение `payUsd`: плоская скидка 50% → номинал ×2.
function pricingForTopup(payUsd: number) {
  return { pay: payUsd, get: Math.round(payUsd / (1 - FLAT_DISCOUNT_PERCENT / 100)), discount: FLAT_DISCOUNT_PERCENT };
}
...
function HeroDiscount() {
  return <aside className="hero-offer reveal">
    ...
    <div className="offer-rate-head">
      ...
      <T k="offer_note_detail" as="p">A flat 50% off for every account</T>
    </div>
