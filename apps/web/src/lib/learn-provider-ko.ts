...
export const learnProviderKo: Record<string, LocalizedContent> = {
  ...
  "kimi-api-quickstart": {
    ...
    description: "apiToken.sale에서 Kimi K3와 Kimi for Coding을 호출하세요. Anthropic Messages, x-api-key, namespaced model ID, streaming, 공유 잔액을 설명합니다.",
    ...
    sections: [
      { h2: "curl로 첫 요청", blocks: [
        ...
        { type: "p", text: "stream: true를 설정하면 증분 SSE를 받습니다. terminal usage가 Anthropic 형식이므로 기존 usage parser를 그대로 사용할 수 있습니다." },
      ] },
    ...
    faq: [
      ...
      { q: "Kimi가 streaming을 지원하나요?", a: "네. stream: true를 설정하고 일반 증분 Anthropic SSE event를 처리합니다." },
  ...
  "kimi-api-for-opencode": {
    ...
    description: "router plugin, live model catalog, 명시적 kimi/* ID, streaming, 하나의 선불 API 키로 OpenCode를 Kimi에 연결하세요.",
