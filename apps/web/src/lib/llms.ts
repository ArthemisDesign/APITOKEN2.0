...
const copy: Record<Locale, LlmsCopy> = {
  en: {
    ...
    summary: "> apiToken.sale is an independent Claude API gateway. It serves the standard Anthropic Messages API and the full supported Claude line (Opus, Sonnet, Haiku) from prepaid balance at a progressive discount of 60% up to 70% off official spend. No Anthropic account, waitlist, or billing-country requirement; pay by bank card or cryptocurrency.",
    ...
    facts: [
      ...
      "Billing: prepaid, per-token at official rates minus a 60–70% B2C discount; balance never expires",
  ...
  ru: {
    ...
    summary: "> apiToken.sale — независимый шлюз к Claude API. Он предоставляет стандартный Anthropic Messages API и всю линейку Claude (Opus, Sonnet, Haiku) с предоплаченного баланса со скидкой 60–70% от официальной стоимости. Без аккаунта Anthropic, без очереди и ограничений по стране; оплата картой или криптовалютой.",
    ...
    facts: [
      ...
      "Оплата за токены по официальным ставкам минус скидка 60–70% для B2C; баланс не сгорает",
  ...
  zh: {
    ...
    summary: "> apiToken.sale 是独立的 Claude API 网关。它提供标准的 Anthropic Messages API 以及完整的 Claude 系列（Opus、Sonnet、Haiku），以预付余额按官方价格的 60%–70% 折扣计费。无需 Anthropic 账户、无需排队、不限国家；支持银行卡或加密货币付款。",
    factsHeading: "## 关键信息",
    facts: [
      "网站: " + SITE_ORIGIN,
      ...
      "计费: 预付、按 token 以官方价格计费再减去 60–70% 的 B2C 折扣；余额永不过期",
      "支付: 银行卡或加密货币",
  ...
  ko: {
    ...
    summary: "> apiToken.sale은 독립적인 Claude API 게이트웨이입니다. 표준 Anthropic Messages API와 전체 Claude 라인업(Opus, Sonnet, Haiku)을 선불 잔액으로 공식 요금 대비 60~70% 할인된 가격에 제공합니다. Anthropic 계정, 대기열, 국가 제한이 없으며 신용카드 또는 암호화폐로 결제할 수 있습니다.",
    factsHeading: "## 핵심 정보",
    facts: [
      "웹사이트: " + SITE_ORIGIN,
      ...
      "과금: 선불, 공식 요금 기준 토큰당 과금에서 B2C 60~70% 할인; 잔액은 만료되지 않음",
      "결제: 신용카드 또는 암호화폐",
