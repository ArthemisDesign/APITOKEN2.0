...
const copy: Record<Locale, LlmsCopy> = {
  en: {
    ...
    summary: "> apiToken.sale is an independent multi-provider API gateway. It serves the standard Anthropic Messages API with the full supported Claude line (Opus, Sonnet, Haiku) and an OpenAI-compatible API (Responses and Chat Completions) with the GPT-5 line — from one prepaid balance and one API key at a flat 50% discount off official spend for every account. No Anthropic or OpenAI account, waitlist, or billing-country requirement; pay by bank card or cryptocurrency.",
    ...
    facts: [
      ...
      "Billing: prepaid, per-token at official rates minus a flat 50% discount shared by both providers; balance never expires",
  ...
  ru: {
    ...
    facts: [
      ...
      "Оплата за токены по официальным ставкам минус плоская скидка 50%, общая для обоих провайдеров; баланс не сгорает",
  ...
  zh: {
    ...
    summary: "> apiToken.sale 是独立的多提供商 API 网关。它以标准 Anthropic Messages API 提供完整的 Claude 系列（Opus、Sonnet、Haiku），并以 OpenAI 兼容 API（Responses 与 Chat Completions）提供 GPT-5 系列——共用一个预付余额和一个 API 密钥，按官方价格的 50% 统一折扣计费。无需 Anthropic 或 OpenAI 账户、无需排队、不限国家；支持银行卡或加密货币付款。",
    factsHeading: "## 关键信息",
    facts: [
      "网站: " + SITE_ORIGIN,
      ...
      "计费: 预付、按 token 以官方价格计费再减去统一 50% 折扣（两家提供商共用）；余额永不过期",
      "支付: 银行卡或加密货币",
  ...
  ko: {
    ...
    summary: "> apiToken.sale은 독립적인 멀티 프로바이더 API 게이트웨이입니다. 표준 Anthropic Messages API로 전체 Claude 라인업(Opus, Sonnet, Haiku)을, OpenAI 호환 API(Responses 및 Chat Completions)로 GPT-5 라인업을 제공합니다 — 하나의 선불 잔액과 하나의 API 키로 공식 요금 대비 50% 할인된 가격입니다. Anthropic이나 OpenAI 계정, 대기열, 국가 제한이 없으며 신용카드 또는 암호화폐로 결제할 수 있습니다.",
    factsHeading: "## 핵심 정보",
    facts: [
      "웹사이트: " + SITE_ORIGIN,
      ...
      "과금: 선불, 공식 요금 기준 토큰당 과금에서 일률 50% 할인(두 프로바이더 공통); 잔액은 만료되지 않음",
      "결제: 신용카드 또는 암호화폐",
...
export function buildLlms(locale: Locale): string {
  const c = copy[locale];
  const modelIds = [...claudeModels, ...openaiModels].map((model) => model.id).join(", ");
  const lines: string[] = [
    c.heading,
    "",
    c.summary,
    ...
    "- Capability parity: on the Anthropic surface, streaming (SSE), tool use / function calling, prompt caching (cache_control) and vision pass through unchanged — identical to the Anthropic Messages API. On the OpenAI-compatible surface, Responses and Chat Completions support SSE streaming and text+image input",
    "",
    c.guidesHeading,
    "",
