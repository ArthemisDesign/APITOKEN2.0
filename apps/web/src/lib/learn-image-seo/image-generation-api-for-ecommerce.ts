...
export const spec: ImageSeoSpec = {
    ...
    title: tr(
      ...
      "API генерации изображений для товарных фото в ecommerce",
      "电商产品图片生成 API",
      "이커머스 상품 사진 생성 API",
    ),
    h1: tr(
    ...
    description: tr(
      "Produce ecommerce product images with Nano Banana 2 or GPT Image 2 through one API key: model routing, real request shapes, identity-safe editing, batch controls, cost math and storefront SEO.",
      "Создавайте product images через Nano Banana 2 или GPT Image 2 с одним API-ключом: routing моделей, реальные request shapes, identity-safe editing, batch controls, расчёт цены и SEO витрины.",
