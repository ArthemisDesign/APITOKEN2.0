...
export const article: LearnArticle = {
  ...
  sections: [
    ...
    { h2: "One family, three meters on the same capabilities", blocks: [
      ...
      { type: "note", text: "Sol's temporary official promotion runs through 2026-11-21 inclusive; standard $5 input and $30 output rates return on 2026-11-22 UTC. The bare model ID gpt-5.6 is an alias of gpt-5.6-sol and follows the same rates. Pin an explicit tier in production config so a silent default never routes routine traffic onto the flagship meter." },
    ] },
    { h2: "When Sol earns its premium over Terra", blocks: [
      ...
      { type: "p", text: "Because output tokens cost five times input on promotional Sol, the cheapest Sol call is a short one. Feed it a tight, well-scoped prompt — the failing test, the relevant diff, the exact question — rather than an unfiltered codebase dump." },
    ] },
    { h2: "When Luna beats Terra on unit economics", blocks: [
      { type: "p", text: "At the temporary Sol rates, Luna costs 5% of Sol input and 6% of Sol output, while remaining a tenth of Terra per token. Any task it completes first try is almost free. Its limits are real, though: depth-sensitive work will fail on Luna and cost you a Terra or Sol retry anyway, which erases the saving. Route to Luna only work that is deterministic, narrow and easy to verify." },
      { type: "list", items: [
    ...
    { h2: "Caching and the 272K boundary can outweigh the tier choice", blocks: [
      { type: "p", text: "Two pricing mechanics move the bill as much as picking the right tier. First, cached input: a repeated prompt prefix bills at the cached rate — $0.40 per 1M on promotional Sol versus $4 fresh, with the same 10% ratio on Terra and Luna — and cache writes bill at 125% of normal input, or $5 per 1M on promotional Sol. In long agent loops that resend the same system prompt and history, a stable prefix compounds into the largest single saving available." },
  ...
  faq: [
    { q: "Which GPT-5.6 model is best for coding?", a: "Start with gpt-5.6-terra: it keeps Sol's 400K context and full reasoning controls while costing 50% of promotional Sol input and 60% of output. Escalate to gpt-5.6-sol for the hardest architecture, debugging or agentic work, and use gpt-5.6-luna for cheap deterministic sub-steps." },
    { q: "How much cheaper is Terra than Sol?", a: "During Sol's temporary promotion, Terra's official $2/$12 input/output rates are 50%/60% of Sol's $4/$20. After the flat 50% discount, Terra is $1/$6 and Sol is $2/$10. The Sol promotion runs through 2026-11-21 inclusive; standard $5/$30 returns on 2026-11-22 UTC." },
    ...
    { q: "Does Terra support the max reasoning effort?", a: "Yes. Sol, Terra and Luna expose the same GPT-5.6 reasoning-effort set — none through xhigh plus max. Reasoning tokens bill as output, so max effort on promotional Sol uses the temporary $20-per-1M official output rate." },
