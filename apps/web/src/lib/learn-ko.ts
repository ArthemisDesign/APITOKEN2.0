...
export const learnKo: Record<string, LocalizedContent> = {
    ...
    sections: [
      ...
      { h2: "결제 방식", blocks: [
        ...
        { type: "list", items: [
          ...
          "B2C 계정은 공식 소비 대비 60% 저렴하게 시작해, 충전이 누적될수록 최대 80% 할인까지 올라갑니다.",
        ] },
      ] },
  ...
  "cheapest-claude-api": {
    title: "가장 저렴한 Claude API — 최대 80% 할인",
    h1: "Claude API를 가장 저렴하게 사용하는 방법",
    description: "Claude API 비용을 최대 80%까지 절감하세요. apitoken.sale은 완전히 동일한 Anthropic Messages API를 선불 할인가로 판매합니다. 같은 모델, 같은 엔드포인트, 더 낮은 토큰당 단가.",
    keywords: ["claude api 저렴", "claude api 할인", "저렴한 claude api", "claude api 가격", "anthropic api 절약", "claude api 싸게"],
    dek: "Claude API는 토큰 단위로 과금되며, 긴 코딩 세션에서는 그 토큰이 빠르게 쌓입니다. apitoken.sale은 선불 잔액을 풀링하고 누진 할인을 적용해 동일한 API를 최대 80% 저렴하게 제공합니다.",
    sections: [
      { h2: "왜 더 저렴한가", blocks: [
        ...
        { type: "list", items: [
          ...
          "누적 충전이 늘어날수록 할인은 최대 80%까지 커집니다.",
  ...
  "free-claude-api-key": {
    ...
    description: "apitoken.sale에서 무료로 Claude API 키를 만들고 공식 API 가격 기준 $10의 Claude 사용량을 받으세요. 카드 불필요, Anthropic 계정 불필요, 즉시 접근.",
    keywords: ["무료 claude api 키", "claude api 무료", "claude api 무료 크레딧", "무료 anthropic api 키", "claude api 카드 없이"],
    dek: "돈을 쓰기 전에 키를 만들고 실제 Claude 호출을 해볼 수 있습니다. 모든 신규 B2C 계정에는 공식 API 가격 기준 $10의 사용량이 지급되어, 연동이 되는지 먼저 검증할 수 있습니다.",
    sections: [
      { h2: "'무료'에 포함된 것", blocks: [
        { type: "list", items: [
          "지원되는 모든 Claude 모델에서 작동하는 API 키.",
          "공식 API 가격 기준 $10의 Claude 사용량, 카드 불필요.",
  ...
  "claude-api-free-trial": {
    ...
    description: "몇 분 만에 Claude로 코딩을 시작하세요. apitoken.sale은 모든 신규 계정에 공식 가격 기준 $10의 Claude 사용량을 지급하며, 카드나 Anthropic 승인이 필요 없습니다.",
    keywords: ["claude api 무료 체험", "claude api 체험판", "claude api 테스트", "claude api 샌드박스", "claude api 데모"],
    dek: "별도로 신청할 체험판이 없습니다. 가입만 하면 공식 API 가격 기준 $10의 사용량을 받아 지원되는 모든 모델을 대상으로 실제 호출을 실행할 수 있습니다.",
    sections: [
    ...
    faq: [
      { q: "체험은 어떻게 시작하나요?", a: "계정만 만들면 됩니다. 공식 가격 기준 $10의 사용량이 신청 절차 없이 자동으로 추가됩니다." },
      { q: "무료 사용량이 소진되면 어떻게 되나요?", a: "달러 단위 정수 금액을 충전하면 계속 사용할 수 있으며, 누진 할인이 즉시 적용됩니다." },
    ],
  },
  ...
  "claude-opus-api": {
    ...
    description: "하나의 apitoken.sale 키로 Claude Opus 4.8 및 4.7을 공식 요율 대비 최대 80% 할인가로 이용하세요. 복잡한 추론, 리팩터, 긴 에이전트 세션에 최적입니다.",
  ...
  "claude-sonnet-api": {
    ...
    description: "apitoken.sale로 Claude Sonnet 5와 Sonnet 4.6을 사용하세요. 일상 코딩과 에이전트의 기본 모델을 공식 API 가격 대비 최대 80% 할인가로 이용할 수 있습니다.",
  ...
  "apitoken-vs-anthropic-direct": {
    ...
    description: "apiToken.sale과 Anthropic 직접 구매 비교: 동일한 Messages API와 모델이지만 최대 80% 할인, 계정 요구 없음, 카드 또는 암호화폐 결제.",
    ...
    faq: [
      ...
      { q: "왜 Anthropic 직접 구매보다 저렴한가요?", a: "잔액이 선불로 풀링되고, 공식 소비에 최대 80%의 누진 할인이 적용되기 때문입니다." },
    ],
  },
  ...
  "claude-api-pricing-explained": {
    ...
    description: "Claude API 가격 이해하기: 토큰당 입력·출력 요율, 프롬프트 캐싱, 그리고 apiToken.sale이 최대 80% 할인을 누진 적용하는 방식.",
  ...
  "how-billing-works": {
    ...
    h1: "과금은 어떻게 작동하는가",
    description: "apiToken.sale 과금 이해하기: 선불 잔액, 공식 요율의 요청 단위 측정, 누진 할인, 대시보드의 토큰 단위 사용량.",
    ...
    sections: [
      ...
      { h2: "요청 단위 측정", blocks: [
        { type: "list", items: [
          ...
          "현재 적용되는 할인(B2C의 경우 60%에서 최대 80%)이 차감됩니다.",
          "순금액이 선불 잔액에서 차감됩니다.",
        ] },
      ] },
      { h2: "완전한 가시성", blocks: [
  ...
  "apitoken-vs-portkey": {
    ...
    sections: [
      { h2: "서로 다른 역할", blocks: [
        ...
        { type: "p", text: "apiToken.sale은 키와 잔액의 원천입니다. https://api.apitoken.sale의 네이티브 Anthropic 엔드포인트를 최대 80% 할인으로, Anthropic 계정 없이 제공합니다." },
      ] },
  ...
  "claude-max-plan-vs-api": {
    ...
    description: "Claude 구독을 쓸 때와 Claude API를 쓸 때. apiToken.sale은 월 요금 없이 모든 모델에 대한 종량제 API 접근을 최대 80% 할인가로 제공합니다.",
    ...
    sections: [
      ...
      { h2: "왜 API가 더 나은 경우가 많은가", blocks: [
        { type: "list", items: [
          ...
          "apiToken.sale은 공식 토큰 요율에서 최대 80%를 더 할인.",
        ] },
        { type: "note", text: "신규 계정은 공식 API 가격 기준 $10의 Claude 사용량으로 시작합니다. 충전 전에 도구를 연결하고 실제 호출을 실행해 보기에 충분한 금액입니다." },
      ] },
    ],
  ...
  "why-choose-apitoken": {
    ...
    description: "개발자들이 Claude에 apiToken.sale을 고르는 이유: 동일한 Anthropic API를 최대 80% 저렴하게, Anthropic 계정 없이 즉시 접근, 카드 또는 암호화폐 결제.",
