...
export const learnZh: Record<string, LocalizedContent> = {
  ...
  "cheapest-claude-api": {
    title: "最便宜的 Claude API——立省 50%",
    h1: "使用 Claude API 最省钱的方式",
    description: "把 Claude API 成本削减 50%。apitoken.sale 以预付折扣价出售一模一样的 Anthropic Messages API——同样的模型、同样的接口、更低的每 token 单价。",
    keywords: ["最便宜的 claude api", "claude api 折扣", "便宜的 claude api", "claude api 价格", "节省 anthropic api 费用", "比 anthropic 更便宜的 claude api"],
    dek: "Claude API 按 token 计费，而在漫长的编码会话中这些 token 累积得很快。apitoken.sale 通过汇集预付余额并套用递进折扣，让你以低 50% 的价格用上完全相同的 API。",
    sections: [
      { h2: "为什么更便宜", blocks: [
      ...
      { h2: "Claude API 折扣如何生效", blocks: [
        ...
        { type: "table", headers: ["模型", "官方 输入 / 输出（$ / 1M）", "本站价（−50%）"], rows: [
    ...
    faq: [
      ...
      { q: "我能省多少？", a: "每个账户一律比官方 API 消费低 50% — 统一固定折扣。" },
      { q: "有没有隐藏费用或订阅？", a: "没有。余额为预付制、永不过期，仅由真实 API 用量消耗——没有月费。" },
      { q: "有比直接从 Anthropic 购买更便宜的 Claude API 吗？", a: "有。apiToken.sale 以统一 50% 折扣出售一模一样的 Anthropic API，且没有订阅。" },
    ],
  },
  ...
  "claude-opus-api": {
    ...
    description: "通过一把 apitoken.sale 密钥以低于官方费率 50% 的价格访问 Claude Opus 4.8 和 4.7。最适合复杂推理、重构与长时间的智能体会话。",
  ...
  "claude-sonnet-api": {
    ...
    description: "通过 apitoken.sale 使用 Claude Sonnet 5 和 Sonnet 4.6——日常编码与智能体的默认模型，以官方 API 价格 5 折使用。",
  ...
  "claude-api-key-for-cursor": {
    ...
    description: "用 apitoken.sale 密钥把 Cursor 接入 Claude：将 Anthropic Base URL 设为 api.apitoken.sale，粘贴密钥，选择模型，即可以50% 折扣编码。",
  ...
  "cursor-without-anthropic-account": {
    ...
    description: "没有 Anthropic 账户？改用 apitoken.sale 密钥在 Cursor 中使用 Claude。即时开通，支持银行卡或加密货币支付，官方费率立省 50%。",
  ...
  "apitoken-vs-anthropic-direct": {
    ...
    description: "对比 apitoken.sale 与 Anthropic 官方直购：完全相同的 Messages API 和模型，但立省 50%、无需账户、支持银行卡或加密货币支付。",
    ...
    faq: [
      ...
      { q: "为什么它比 Anthropic 官方直购更便宜？", a: "余额是预付且汇集的，并对官方消费套用统一 50% 折扣。" },
    ],
  },
  ...
  "claude-api-pricing-explained": {
    ...
    description: "了解 Claude API 定价：按 token 的输入与输出费率、提示缓存，以及 apitoken.sale 如何套用统一 50% 折扣。",
    ...
    sections: [
      ...
      { h2: "apitoken.sale 的折扣", blocks: [
        { type: "p", text: "每次调用先换算为官方 Anthropic 消费，再减去统一的 50% 折扣。每次请求都在控制台中以 token 级别的明细可见。" },
        { type: "note", text: "通过 Google 或 GitHub 创建的新账户可获价值 $10 的 Claude 官方价格用量；邮箱密码账户不享受此奖励。" },
      ] },
    ...
    faq: [
      ...
      { q: "折扣如何套用？", a: "先计算官方消费，再在扣减余额前减去统一的 50% 折扣。" },
      { q: "Claude API 的 token 如何计价？", a: "按 token 计费，分输入和输出，缓存读取更便宜。apiToken.sale 在官方 token 费率之上再套用你 50% 的折扣。" },
    ],
  },
  ...
  "apitoken-vs-proxyapi": {
    ...
    faq: [
      { q: "apiToken.sale 比普通转售商更便宜吗？", a: "它对官方 Claude 消费套用统一 50% 折扣，而不是在标价之上再加价。" },
  ...
  "apitoken-vs-portkey": {
    ...
    sections: [
      { h2: "各司其职", blocks: [
        ...
        { type: "p", text: "apitoken.sale 才是密钥和余额的来源：一个位于 https://api.apitoken.sale 的原生 Anthropic 端点，立省 50%，且无需 Anthropic 账户。" },
      ] },
      { h2: "两者甚至可以组合", blocks: [
    ...
    sections: [
      { h2: "自托管 vs 托管", blocks: [
        { type: "list", items: [
          ...
          "apiToken.sale 对 Claude 消费提供 50% 的折扣，这是裸代理做不到的。",
        ] },
        { type: "note", text: "通过 Google 或 GitHub 创建的新账户可获价值 $10 的 Claude 官方价格用量；邮箱密码账户不享受此奖励。" },
      ] },
      { h2: "分别适合什么时候用", blocks: [
  ...
  "claude-max-plan-vs-api": {
    ...
    description: "何时该用 Claude 订阅、何时该用 Claude API。apiToken.sale 提供按量付费的全模型 API 权限，无月费，立省 50%。",
    ...
    sections: [
      ...
      { h2: "为什么 API 往往更胜一筹", blocks: [
        { type: "list", items: [
          ...
          "apiToken.sale 在官方 token 费率上再享50% 折扣。",
        ] },
        { type: "note", text: "通过 Google 或 GitHub 创建的新账户可获价值 $10 的 Claude 官方价格用量；邮箱密码账户不享受此奖励。" },
      ] },
    ],
  ...
  "why-choose-apitoken": {
    ...
    description: "开发者选择 apiToken.sale 使用 Claude 和 GPT 的理由：官方 Anthropic 与 OpenAI 兼容 API 便宜 50%，无需提供商账户即时开通，支持银行卡或加密货币支付。",
    ...
    sections: [
      { h2: "一句话版本", blocks: [
        { type: "list", items: [
          ...
          "在永不过期的预付余额上，官方消费立省 50%——同一条折扣轨道覆盖两家提供商。",
      ...
      { h2: "同一余额上的折扣 API token", blocks: [
        { type: "p", text: "把它当作打折出售的 Claude 和 GPT API token：你一次性充值余额，享受官方 token 费率50% 的折扣，并在两家提供商的所有模型和工具上使用。余额永不过期，也没有订阅。" },
      ] },
    ],
    faq: [
      { q: "apiToken.sale 有什么不同？", a: "它是同一套 Claude 和 GPT API，便宜 50%，即时开通、无需提供商账户，支持银行卡或加密货币付款。" },
      { q: "API 有任何改动吗？", a: "没有——协议、模型和响应都是标准的 Anthropic 与 OpenAI 兼容版本。只有价格和开通方式不同。" },
      { q: "apiToken.sale 是什么？", a: "一个独立的多提供商 API 网关，以折扣价预付出售官方 Anthropic API 和 OpenAI 兼容 API 的访问权——同样的模型，便宜 50%，无需提供商账户。" },
    ],
  },
  "claude-api-gateway": {
    title: "什么是 Claude API 网关？",
    h1: "Claude API 网关是什么",
    description: "Claude API 网关位于你的工具和 Anthropic 之间，增加接入、计费和管控能力。apitoken.sale 是一个带 50% 折扣的原生网关。",
  ...
  "claude-code-api-key": {
    ...
    description: "只需两个环境变量即可为 Claude Code 配置 apitoken.sale 密钥，用预付余额以50% 折扣运行所有 Claude 模型。",
  ...
  "openai-api-quickstart": {
    ...
    description: "通过 apiToken.sale 的 OpenAI 兼容 API 运行 GPT-5.6 模型——Responses 与 Chat Completions 支持 SSE 流式输出，一个 sk-pool 密钥与 Claude 共用余额，享 50% 折扣。",
    keywords: ["openai 兼容 api", "gpt-5.6 api", "responses api", "chat completions 自定义 base url", "openai sdk base_url", "gpt api 密钥", "gpt-5.6 价格"],
    dek: "你的 sk-pool 密钥不只是 Claude 专用。同一个密钥和预付余额还通过 OpenAI 兼容端点提供 GPT-5 系列——标准的 Responses 与 Chat Completions 调用、官方 OpenAI SDK、SSE 流式输出，以及同样的 50% 折扣。",
    sections: [
      ...
      { h2: "可用的 GPT 模型", blocks: [
        ...
        { type: "list", items: [
          ...
          "你的 B2C 折扣与 Claude 用量完全一致——一个余额、一个档位，按官方费用 50% 折扣。",
        ] },
    ...
    faq: [
      ...
      { q: "GPT 用量如何计费？", a: "按官方 OpenAI 费率逐 token 计费——包括缓存输入和长上下文定价——然后在计入预付余额前减去你当前的 统一 50% 折扣，与 Claude 用量完全一致。" },
    ],
  },
  "codex-cli-setup": {
    title: "用 apiToken.sale 配置 Codex CLI——GPT-5.6 配置档",
    h1: "在 apiToken.sale 上运行 Codex CLI",
    description: "用指向 apiToken.sale OpenAI 兼容端点的命名 model_providers 配置档配置 Codex CLI——GPT-5.6 模型按预付余额50% 折扣计费，无需 ChatGPT 账户。",
    ...
    dek: "只要给 Codex CLI 一个自定义模型提供商，它就能完全以 API 密钥认证运行。一个 TOML 配置档把它指向 apiToken.sale，预付余额覆盖每次会话——无需登录 ChatGPT，比官方费用便宜 50%。",
    sections: [
      { h2: "创建配置档", blocks: [
    ...
    faq: [
      ...
      { q: "折扣与 Claude 相同吗？", a: "相同。GPT-5.6 用量按官方 OpenAI token 费率计量，统一的 50% 折扣作用于同一个预付余额。" },
  ...
  "claude-api-aider": {
    ...
    description: "通过 apitoken.sale 在 Claude 上运行 Aider：导出 ANTHROPIC_API_BASE 和密钥，选一个 Claude 模型，以 50% 的折扣在终端结对编程。",
    ...
    faq: [
      ...
      { q: "长 Aider 会话能便宜多少？", a: "每个请求按官方 token 费率计费再减去你 50% 的折扣，直连要花 $10 的会话在这里只需 $3–4。" },
    ],
  },
  "claude-api-roo-code": {
    title: "在 Roo Code 中使用 Claude API",
    h1: "在 Roo Code 中使用 Claude API",
    description: "通过 apitoken.sale 将 VS Code 中的 Roo Code 接入 Claude：选择 Anthropic 提供方，启用自定义 base URL，粘贴密钥，以 50% 的折扣编码。",
