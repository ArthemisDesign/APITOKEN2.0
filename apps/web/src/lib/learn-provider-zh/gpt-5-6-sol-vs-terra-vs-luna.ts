...
export const content: LocalizedContent = {
    ...
    dek: "GPT-5.6 Sol、Terra、Luna 是同一模型家族的三个价位：同样 400K 上下文、128K 输出上限和推理控制，token 单价从 $0.20/$1.20 到 $5/$30（每 1M）。对大多数负载，正确答案是以 Terra 为默认档、Sol 作为升级档、Luna 承接大批量的机械性步骤——在 apiToken.sale 上，三档共用一把密钥、同一个预付余额，全线五折。",
    sections: [
      { h2: "简短答案：默认 Terra，Sol 和 Luna 守两端", blocks: [
        { type: "p", text: "几乎所有任务都用 gpt-5.6-terra，确实需要更深推理时再升级到 gpt-5.6-sol，把可预测的批量工作下放给 gpt-5.6-luna。Terra 保留了完整的 400K 上下文窗口、128K 输出上限和全部推理强度档位，token 价格却只有 Sol 的 40%，因此是编程、生产对话和 Agent 循环的正确默认档。" },
      ...
      { h2: "一个家族，同一能力上的三种计价", blocks: [
        ...
        { type: "table", headers: ["", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"], rows: [
          ["官方输入 / 输出", "$5 / $30", "$2 / $12", "$0.20 / $1.20"],
          ["本站统一五折后", "$2.50 / $15", "$1 / $6", "$0.10 / $0.60"],
          ["缓存输入（官方）", "$0.50", "$0.20", "$0.02"],
          ["上下文窗口", "400K tokens", "400K tokens", "400K tokens"],
          ["最大输出", "128K tokens", "128K tokens", "128K tokens"],
          ["推理强度", "none → max", "none → max", "none → max"],
          ["角色", "升级档", "日常主力", "走量档"],
        ] },
        { type: "note", text: "裸模型 ID gpt-5.6 是 gpt-5.6-sol 的别名——它按 Sol 的费率计费，而不是家族的某种均价。在生产配置里写死明确档位，别让沉默的默认值把日常流量悄悄路由到旗舰计价上。" },
      ] },
      { h2: "什么时候 Sol 配得上 Terra 2.5 倍的计价", blocks: [
        ...
        { type: "p", text: "Sol 上输出 token 的价格是输入的六倍，所以最便宜的 Sol 调用是短调用。给它紧凑、边界清晰的提示词——失败的测试、相关的 diff、确切的问题——而不是未经筛选的代码库大杂烩。" },
      ] },
      { h2: "什么时候 Luna 在单位经济上赢过 Terra", blocks: [
        { type: "p", text: "Luna 的单价是 Sol 的 4%、Terra 的十分之一，所以任何它一次做对的任务几乎免费。但它的限制是真实存在的：吃深度的工作在 Luna 上会失败，最终还是得用 Terra 或 Sol 重试，省下的钱就被抹掉了。只把确定性、窄口径、易验证的工作路由给 Luna。" },
        { type: "list", items: [
          "生产流量中的分类、打标、路由和意图识别。",
      ...
      { h2: "缓存和 272K 分界线的影响可能超过选档", blocks: [
        { type: "p", text: "有两个计费机制对账单的影响不亚于选对档位。第一是缓存输入：重复的提示词前缀按缓存费率计费——Sol 上每 1M $0.50，而新输入是 $5，Terra 和 Luna 同样是 10% 的比例；写入缓存则按普通输入的 125% 计费。在反复发送同一系统提示词和历史的长 Agent 循环里，稳定的前缀会累积成你能拿到的最大单笔节省。" },
        { type: "p", text: "第二是长上下文阶梯：输入超过 272K token 后，整个请求按 2 倍输入、1.5 倍输出重新计价——不只是超出部分。一个 273K token 的请求比 270K 的贵一倍还多。不管在哪一档，越线之前先把超大上下文拆开或裁剪历史。" },
        { type: "note", text: "推理 token 按输出 token 计费。在 Sol 上把强度拉满到 max，意味着为「认真思考」付每 1M $30（官方价）——当推理本身就是产出时值得，花在机械任务上就是浪费。让强度匹配档位：便宜档开高强度，往往不如强档开低强度划算。" },
      ] },
    ...
    faq: [
      { q: "哪款 GPT-5.6 模型最适合编程？", a: "从 gpt-5.6-terra 开始：它保留 Sol 的 400K 上下文和完整推理控制，token 价格只有 40%。最难的架构、调试或 Agent 任务升级到 gpt-5.6-sol，廉价的确定性子步骤用 gpt-5.6-luna。" },
      { q: "Terra 比 Sol 便宜多少？", a: "官方价每 1M 输入/输出 token 为 $2/$12，对比 Sol 的 $5/$30——旗舰费率的 40%。apiToken.sale 的统一五折对两者同样适用：Terra 为 $1/$6，Sol 为 $2.50/$15。" },
      { q: "Sol、Terra、Luna 用不同的端点或密钥吗？", a: "不用。三者跑在同一个 OpenAI 兼容 base URL 上，用同一把 Bearer 密钥、同一个预付余额；请求里只有模型 ID 不同。" },
      { q: "Terra 支持 max 推理强度吗？", a: "支持。Sol、Terra、Luna 暴露同一套 GPT-5.6 推理强度——none 到 xhigh 加 max。推理 token 按输出计费，所以 Sol 上开 max 走的就是每 1M $30 的输出费率。" },
