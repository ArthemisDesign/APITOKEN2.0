    ...
    sections: [
      { h2: "Gemini API 每 100 万 token 价格：逐模型对比", blocks: [
        ...
        { type: "table", headers: ["模型", "官方 输入 / 缓存 / 输出", "本站五折后价格"], rows: [
          ...
          ["gemini-3.6-flash", "$0.75 / $0.075 / $3.75 促销价", "$0.375 / $0.0375 / $1.875"],
      ...
      { h2: "一次真实调用的三条 token 计费项如何叠加", blocks: [
        { type: "p", text: "任何一次调用的成本都是三项乘积之和：新输入 token 数乘以输入费率，缓存 token 数乘以缓存费率，输出 token 数乘以输出费率。在 Google 促销期（截至 2026-12-31），一次向 gemini-3.6-flash 发送 20,000 个输入 token（其中 12,000 个命中缓存）、生成 1,500 个输出 token 的请求，按官方费率为 8,000 × $0.75/M + 12,000 × $0.075/M + 1,500 × $3.75/M = $0.006 + $0.0009 + $0.005625 = $0.012525。固定五折之后，这次调用实际结算 $0.0062625。" },
        { type: "list", items: [
      ...
      { h2: "输入超过 200K token 的长上下文定价", blocks: [
        ...
        { type: "p", text: "Flash 和 Flash-Lite 没有这一档。它们在整个 100 万 token 上下文窗口内（输出上限 64K）都保持标准费率。一个 50 万 token 的分析任务，仅输入在 Pro 上就要 $2；按 Gemini 3.6 Flash 当前官方促销输入费率只需 $0.375——然后 apiToken.sale 五折再把这两个数字各砍一半。" },
