...
export const content: LocalizedContent = {
    ...
    sections: [
      ...
      { h2: "这把密钥能用哪些 GPT 模型", blocks: [
        ...
        { type: "table", headers: ["模型 ID", "层级", "官方 输入 / 缓存 / 输出（每 1M token，$）", "五折后"], rows: [
          ["gpt-5.6-sol（别名：gpt-5.6）", "旗舰", "$4 / $0.40 / $20", "$2 / $0.20 / $10"],
        ...
        { type: "list", items: [
          "Sol 临时官方输入/缓存/缓存写入/输出费率截至 2026-11-21（含当日）为 $4/$0.40/$5/$20，本站五折后为 $2/$0.20/$2.50/$10；自 2026-11-22 UTC 起恢复标准输入 $5、输出 $30。",
      ...
      { h2: "预付余额与五折如何结算", blocks: [
        ...
        { type: "list", items: [
          "缓存输入单独计价，远比新鲜输入便宜（Sol 促销期每 1M 是 $0.40 对 $4），所以在多次调用之间保持稳定的提示词前缀，省的是真金白银。",
          "缓存写入按普通输入的 125% 计费；缓存读取按输入的 10% 计费。",
          "推理 token 计入输出用量，不会作为独立计费项再收第二次。",
        ] },
        { type: "note", text: "输入 token 超过 272K 后，长上下文费率对整个请求生效——输入 2 倍、输出 1.5 倍，且在折扣之前。按 Sol 促销价，270K 输入加 2K 输出官方成本为 $1.12，273K 加 2K 为 $2.244；在越过边界之前，先拆分超大上下文。" },
    ...
    faq: [
      ...
      { q: "这里 GPT-5.6 每 1M token 多少钱？", a: "截至 2026-11-21（含当日），Sol 临时官方输入/缓存/缓存写入/输出费率为 $4/$0.40/$5/$20，本站结算为 $2/$0.20/$2.50/$10。自 2026-11-22 UTC 起恢复标准输入 $5、输出 $30。Terra 官方价仍为 $2/$12，Luna 为 $0.20/$1.20。" },
