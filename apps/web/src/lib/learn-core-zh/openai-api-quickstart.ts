  ...
  sections: [
    ...
    { h2: "模型 ID、逐 token 价格与 272K 陷阱", blocks: [
      ...
      { type: "list", items: [
        ...
        "缓存输入单独定价，远低于新输入（旗舰上每 1M 为 $0.50 对 $5）——在多次调用间保持稳定的提示词前缀，省的是真金白银，而不是微优化。",
        "你的统一 50% B2C 折扣在这里的作用方式与 Claude 用量完全一致——一个余额、一个费率，官方费用减半。",
      ] },
      { type: "note", text: "272K 阈值就是那个陷阱：一旦超过，OpenAI 长上下文费率会对整个请求生效——输入 2 倍、输出 1.5 倍，而不只是超出部分。273K 输入 token 的请求，花费是 270K 的两倍多。在越过边界之前，拆分过大的上下文或裁剪历史记录。" },
