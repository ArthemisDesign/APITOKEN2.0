...
gate, берёт машинный merge-lock, отказывается вставать в очередь за красным или ещё деплоящимся
`master`, ребейзит, перепроверяет gate на том самом SHA, который пушит, и держит lock до зелёного
`deploy/watchdog`. Мёржить вручную запрещено. Красный SHA не ретраить — исправлять новым коммитом
на новой ветке.
