...
export const MIGRATION_DATABASE_URL_ENV = "DATABASE_URL";
export const MIGRATION_LOCK_TIMEOUT_ENV = "DB_MIGRATION_LOCK_TIMEOUT_MS";
export const MIGRATION_STATEMENT_TIMEOUT_ENV = "DB_MIGRATION_STATEMENT_TIMEOUT_MS";
export const MIGRATION_LOCK_KEY = "719471115124720130";
...
export async function runMigrations(env: NodeJS.ProcessEnv = process.env): Promise<void> {
  ...
  try {
    ...
    console.info("acquiring migration lock");
    await client.query("SELECT pg_advisory_lock($1::bigint)", [MIGRATION_LOCK_KEY]);
    lockAcquired = true;
    console.info("acquired migration lock");
