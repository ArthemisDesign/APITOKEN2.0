...
/**
 * Models an OpenKeys batch may be issued against. OpenKeys sells the official rate one-to-one, so
 * this is a plain published-identity list — the immutable catalog/capability generations that used
 * to carry it were part of the retired pricing release machinery and are gone.
 */
export const OPENKEYS_SUPPORTED_MODELS = Object.freeze([
  "claude-fable-5",
  "claude-haiku-4-5",
  "claude-opus-4-7",
  "claude-opus-4-8",
...
/** A verified admin operator id from the `x-admin-actor` header. */
export const adminActorSchema = z.string().min(1).max(200)
  .refine((value) => !/[\u0000-\u001f\u007f-\u009f]/u.test(value), "actor contains control characters");

/** The human reason an admin mutation records in the audit log. */
export const adminReasonSchema = z.string().trim().min(3).max(2_000)
...
export const createBusinessInviteSchema = z.object({
  ...
  // The invited customer's negotiated discount. One number is the whole pricing decision; the
  // retired rule-set editor is gone, so there is no second shape to reconcile it against.
