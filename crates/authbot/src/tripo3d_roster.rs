//! Atomic publication of acquired Tripo3D (VAST / Holymolly) API-platform profiles into the
//! engine roster.
//!
//! Separate from `tripo3d_key` on purpose: that module owns provider validation, this one
//! owns the filesystem contract. Contract: `docs/engine/PROVIDER_ONBOARDING.md` §6; provider
//! facts: `docs/engine/TRIPO3D_PROVIDER.md` §2.
//!
...
//!
//! The one Tripo3D-specific rule here is key uniqueness. Tripo3D has no machine-readable
//! `/me`, so the static API key itself is the quota identity
//! (`docs/engine/TRIPO3D_PROVIDER.md` §2). Two profiles carrying the same key would
//! double-count one subscription's capacity and split its calibration evidence, so
//! re-publishing an already-known key replaces its existing profile in place rather than
...
use tripo3d_credential::{
    decode_envelope, encode_envelope, validate_profile_id, CredentialKeyring, Tripo3dCredential,
};
...
pub fn publish(
    ...
    let (target_id, credential_path, replaced_existing) = match replacement {
        // Re-publication of a known key. Rotation at Tripo3D means reissuing the key in the
        // console, and the seller flow for a reissued key arrives as a *different* key; the
        // same key re-validated refreshes cohort/base/proxy material in place, so refusing
...
fn private_dir(path: &Path) -> Result<()> {
    fs::create_dir_all(path).context("create Tripo3D credential directory")?;
    let metadata = fs::symlink_metadata(path).context("stat Tripo3D credential directory")?;
    if metadata.file_type().is_symlink() || !metadata.is_dir() {
        bail!("Tripo3D credential directory must be a real directory");
    }
...
fn read_private(path: &Path) -> Result<Vec<u8>> {
    ...
    {
        bail!("Tripo3D file must be a private regular non-symlink file");
    }
...
fn write_new_private(path: &Path, bytes: &[u8]) -> Result<()> {
    let mut file = OpenOptions::new()
        ...
        .mode(0o600)
        .open(path)
        .context("create private Tripo3D file")?;
...
fn atomic_private_replace(path: &Path, bytes: &[u8]) -> Result<()> {
    let parent = path.parent().context("Tripo3D roster has no parent")?;
    let mut random = [0u8; 8];
    getrandom::fill(&mut random).map_err(|_| anyhow::anyhow!("CSPRNG unavailable"))?;
    let staging = parent.join(format!(".tripo3d.{}.pending", URL_SAFE_NO_PAD.encode(random)));
    write_new_private(&staging, bytes)?;
    if let Err(error) = fs::rename(&staging, path) {
        let _ = fs::remove_file(&staging);
        return Err(error).context("publish Tripo3D roster");
    }
...
mod tests {
    use super::*;
    use tripo3d_credential::{Tripo3dCredentialKind, TRIPO3D_BASE_URL_GLOBAL};
    ...
    fn credential(key: &str) -> Tripo3dCredential {
        credential_full(key, "tripo3d api $50", "")
    }

    fn credential_full(key: &str, cohort: &str, proxy: &str) -> Tripo3dCredential {
        Tripo3dCredential {
            version: 1,
    ...
    fn root() -> PathBuf {
        ...
        let path =
            std::env::temp_dir().join(format!("tripo3d-roster-{}", URL_SAFE_NO_PAD.encode(random)));
    ...
    fn publication_writes_a_private_envelope_and_an_atomic_roster() {
        ...
        let ring = keyring();
        let published =
            publish(&root, &ring, "a1", "tripo3d-01", &credential("tsk_key-1")).unwrap();
        assert!(!published.replaced_existing);
        assert_eq!(published.id, "tripo3d-01");
        ...
        assert_eq!(roster.profiles[0].id, "tripo3d-01");
        fs::remove_dir_all(&root).ok();
    }

    #[test]
    fn the_roster_never_carries_the_key_the_cohort_or_the_proxy() {
        let root = root();
        let ring = keyring();
        publish(
            &root,
        ...
        assert!(!roster.contains("tsk_secret-key-9f8c7b"));
        assert!(!roster.contains("tripo3d api $100"));
    ...
    fn the_same_key_replaces_its_profile_instead_of_adding_a_second_one() {
        let root = root();
        let ring = keyring();
        let first = publish(&root, &ring, "a1", "tripo3d-01", &credential("tsk_key-1")).unwrap();
        ...
        let second = publish(
            ...
            "a1",
            "tripo3d-99",
            &credential_full("tsk_key-1", "tripo3d api $100", ""),
        )
        .unwrap();
        ...
        assert_eq!(opened.api_key, "tsk_key-1");
        assert_eq!(opened.cohort, "tripo3d api $100");
        fs::remove_dir_all(&root).ok();
    }
    ...
    fn two_different_keys_get_two_profiles() {
        ...
        let ring = keyring();
        publish(&root, &ring, "a1", "tripo3d-01", &credential("tsk_key-1")).unwrap();
        publish(&root, &ring, "a1", "tripo3d-02", &credential("tsk_key-2")).unwrap();
        let roster: ProfilesFile =
            serde_json::from_slice(&fs::read(root.join("profiles.json")).unwrap()).unwrap();
    ...
    fn a_reused_profile_id_for_a_new_key_is_refused() {
        ...
        publish(&root, &ring, "a1", "tripo3d-01", &credential("tsk_key-1")).unwrap();
        assert_eq!(
            publish(&root, &ring, "a1", "tripo3d-01", &credential("tsk_key-2")),
    ...
    fn a_roster_pointing_outside_the_credential_directory_fails_closed() {
        ...
        assert_eq!(
            publish(&root, &ring, "a1", "tripo3d-02", &credential("tsk_key-2")),
    ...
    fn a_symlinked_envelope_stops_publication() {
        ...
        let elsewhere = std::env::temp_dir()
            .join(format!("tripo3d-elsewhere-{}", URL_SAFE_NO_PAD.encode(random)));
    ...
    fn an_invalid_credential_never_touches_the_roster() {
        let root = root();
        let ring = keyring();
        let mut broken = credential("tsk_key-1");
        broken.api_key = String::new();
        assert_eq!(
            publish(&root, &ring, "a1", "tripo3d-01", &broken),
            Err(PublishError::Storage)
        );
    ...
    fn publication_leaves_no_staging_files_behind() {
        ...
        for dir in [root.clone(), root.join("credentials")] {
            let leftovers: Vec<_> = fs::read_dir(&dir)
                ...
                .filter(|entry| entry.file_name().to_string_lossy().starts_with(".tripo3d."))
                .collect();
