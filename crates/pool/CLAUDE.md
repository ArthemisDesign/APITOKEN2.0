...
приговор (мог быть битый запрос клиента / транзиент), нужна серия `DEAD_STREAK`(3) чистых probe за
≥`DEAD_MIN_SECS`(30мин). `Dead` ИСКЛЮЧАЕТСЯ из `route`/`pick`/`place_best` (гарантированный 401 клиенту
