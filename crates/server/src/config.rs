...
fn gemini_batch_config() -> Option<GeminiBatchRuntimeSettings> {
    if !ev_bool("CLAUDE_API_GEMINI_BATCH_ENABLED", false) {
        return None;
    }
    let specification = ev("CLAUDE_API_GEMINI_BATCH_DATA_KEYS").unwrap_or_else(|| {
        panic!("CLAUDE_API_GEMINI_BATCH_DATA_KEYS is required when Gemini Batch is enabled")
    });
    ...
    Some(GeminiBatchRuntimeSettings {
        public_enabled: ev_bool("CLAUDE_API_GEMINI_BATCH_PUBLIC_ENABLED", false),
        data_keys,
        runtime: forward::gemini::GeminiBatchRuntimeConfig {
            ...
            idle_backoff: Duration::from_secs(bounded_u64(
                "CLAUDE_API_GEMINI_BATCH_IDLE_SECS",
                1,
                1,
                60,
            )),
            retry_backoff: Duration::from_secs(bounded_u64(
                "CLAUDE_API_GEMINI_BATCH_RETRY_SECS",
                5,
                1,
                3600,
            )),
...
fn strict_mib_value(
    key: &str,
    value: Option<&str>,
    default: api_limits::ByteLimit,
) -> Result<api_limits::ByteLimit, String> {
    value.map_or(Ok(default), |value| {
        api_limits::parse_decimal_mib(value).map_err(|error| format!("{key}={value:?}: {error}"))
    })
}
...
fn parse_body_limits(values: &BTreeMap<String, String>) -> Result<api_limits::BodyLimits, String> {
    ...
    let limits = api_limits::BodyLimits {
        ...
        spool_budget: read(
            "CLAUDE_API_BODY_SPOOL_BUDGET_MIB",
            api_limits::current::PROVIDER_SPOOL_BUDGET,
        )?,
        memory_threshold: read(
            "CLAUDE_API_BODY_MEMORY_THRESHOLD_MIB",
            api_limits::current::PROVIDER_MEMORY_THRESHOLD,
        )?,
        response: read(
            "CLAUDE_API_NONSTREAM_RESPONSE_MAX_MIB",
...
fn gemini_config() -> Option<GeminiConfig> {
    ...
    Some(GeminiConfig {
        ...
        rate_limit_rpm_cool_secs: bounded_i64(
            "CLAUDE_API_GEMINI_RATE_LIMIT_RPM_COOL_SECS",
            2,
            1,
            60,
        ),
...
impl Settings {
    pub fn from_env() -> Self {
        ...
        let gemini_batch = if provider.serves_gemini() {
            gemini_batch_config()
        } else {
            None
        };
