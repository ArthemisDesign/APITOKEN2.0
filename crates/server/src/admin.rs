...
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Stage8EngineEvidenceCaptureReq {
    target_generation: i64,
    recovery_generation: i64,
    window_start_ts: i64,
    window_end_ts: i64,
    min_samples_per_provider: i64,
    financial_sample_size: i64,
    gemini_client_admissions: i64,
}

impl Stage8EngineEvidenceCaptureReq {
    fn into_registry(
        self,
        runtime_manifest: registry::pricing::PricingRuntimeManifestEvidence,
    ) -> Result<registry::stage8::Stage8EngineEvidenceRequest, Response> {
        let invalid = |message: &'static str| {
            (StatusCode::BAD_REQUEST, Json(json!({"error": message}))).into_response()
        };
        if self.target_generation <= 0 || self.recovery_generation <= self.target_generation {
            return Err(invalid(
                "target_generation must be positive and recovery_generation must be newer",
            ));
        }
        ...
        let now = std::time::SystemTime::now()
            ...
            .map(|duration| duration.as_secs() as i64)
            .unwrap_or(0);
        if self.window_end_ts > now {
            return Err(invalid("Stage 8 evidence window cannot end in the future"));
        }
        if !(1..=1_000_000).contains(&self.min_samples_per_provider) {
            return Err(invalid(
                "min_samples_per_provider must be between 1 and 1000000",
            ));
        }
        if !(1..=1_000).contains(&self.financial_sample_size) {
            return Err(invalid("financial_sample_size must be between 1 and 1000"));
        }
        if self.gemini_client_admissions < 0 {
            return Err(invalid("gemini_client_admissions cannot be negative"));
        }

        Ok(registry::stage8::Stage8EngineEvidenceRequest {
            target_generation: self.target_generation,
            recovery_generation: self.recovery_generation,
            window_start_ts: self.window_start_ts,
            window_end_ts: self.window_end_ts,
            min_samples_per_provider: self.min_samples_per_provider,
            financial_sample_size: self.financial_sample_size as usize,
            gemini_client_admissions: self.gemini_client_admissions,
            runtime_manifest,
        })
    }
}

/// POST /admin/pricing/v2/stage8-evidence/capture — read-only full engine evidence snapshot.
pub async fn capture_stage8_engine_evidence(
    State(app): State<AppState>,
    Json(request): Json<Stage8EngineEvidenceCaptureReq>,
) -> Response {
    let request = match request.into_registry((*app.pricing_manifest).clone()) {
        Ok(request) => request,
        Err(response) => return response,
    };
    let billing = match billing(&app) {
    ...
    match billing.stage8_engine_evidence(request).await {
        // A blocked report is still durable evidence. `passed=false` is not an HTTP failure and
        // must reach the protected consumer without filtering, with exact serde integer encoding.
        Ok(report) => stage8_engine_evidence_response(report),
        Err(error) => authority_unavailable("Stage 8 engine evidence capture", error),
    }
}

fn stage8_engine_evidence_response(
    report: registry::stage8::Stage8EngineEvidenceReport,
) -> Response {
...
#[cfg(test)]
mod stage8_response_tests {
    use super::stage8_engine_evidence_response;
    use axum::body::to_bytes;
    use axum::http::StatusCode;
    use registry::stage8::{
        Stage8EngineEvidenceCounts, Stage8EngineEvidenceReport, Stage8EvidenceBlocker,
        Stage8ReleasePairEvidence, Stage8RuntimeManifestEvidence,
    };
    use std::collections::BTreeMap;

    #[tokio::test]
    async fn blocked_report_is_still_an_unwrapped_successful_capture() {
        let report = Stage8EngineEvidenceReport {
            schema_version: 2,
            captured_ts: 100,
            window_start_ts: 10,
            window_end_ts: 20,
            ...
            passed: false,
            release: Stage8ReleasePairEvidence {
                target_generation: 41,
                target_digest: None,
                recovery_generation: 42,
                recovery_digest: None,
                recovery_link_digest: None,
                inventory_digest: None,
                funding_digest: None,
                target_assignment_count: 0,
                recovery_assignment_count: 0,
                active_head: None,
            },
            runtime_manifest: Stage8RuntimeManifestEvidence {
                generation: 1,
                digest: "runtime-digest".to_owned(),
            ...
            counts: Stage8EngineEvidenceCounts {
                total_accounts: 0,
                active_accounts: 0,
                account_classes: BTreeMap::new(),
                reconciled_accounts: 0,
                snapshots_by_provider: BTreeMap::new(),
                evaluations_by_outcome: BTreeMap::new(),
                comparisons: BTreeMap::new(),
                scalar_parity_rows: 0,
                policy_divergence_rows: 0,
                gemini_usage_rows: 0,
                gemini_outbox_rows: 0,
                live_runtime_instances: 0,
                release_capable_runtime_instances: 0,
                legacy_inflight_reservations: 0,
                legacy_inflight_outbox_rows: 0,
            },
            financial_samples: Vec::new(),
            engine_inventory_digest: "inventory-digest".to_owned(),
            funding_digest: "funding-digest".to_owned(),
            shadow_digest: "shadow-digest".to_owned(),
            runtime_floor_digest: "runtime-floor-digest".to_owned(),
            legacy_inflight_count: 0,
            blockers: vec![Stage8EvidenceBlocker {
                code: "target_release_missing".to_owned(),
                count: 1,
                subject_digests: vec!["subject-digest".to_owned()],
            }],
            evidence_digest: "evidence-digest".to_owned(),
        };

        let response = stage8_engine_evidence_response(report);
        assert_eq!(response.status(), StatusCode::OK);
        let body = to_bytes(response.into_body(), 64 * 1024).await.unwrap();
        let body: serde_json::Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(body["schema_version"], 2);
        assert_eq!(body["passed"], false);
        assert_eq!(body["blockers"][0]["code"], "target_release_missing");
