...
- `http.rs` — роутер: `/health`, `/pool`, `/balance`, `/capacity` (управляющие) + `/panel`
  (существующий HTML-дашборд, `panel.html` через include_str!) + `/admin-panel` (отдельный
  `admin-panel.html` для admin.apitoken.sale; архитектура обеих панелей — корневой `PANEL.md`) +
