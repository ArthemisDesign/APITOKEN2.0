...
fn postgres_stage8_engine_evidence_contract() {
    use crate::pricing::{
        ...
        FundingEnforcement, LegacyScalarReserveOutcome, PolicyActiveExpectation, PolicyEnforcement,
        PricingMutation, ReconciliationState,
    };
    ...
    let funding_manifest_digest =
        crate::stage8::funding_manifest_digest(&mut pg.client, Some(&release_for_funding)).unwrap();
    pg.client
        .batch_execute(
        ...
        handles.push(std::thread::spawn(move || {
            ...
            for _ in 0..5 {
                match contender.activate_pricing_release_v2(&activation, &runtime_manifest) {
                    ...
                    Err(error) if format!("{error:#}").contains("could not serialize access") => {
                        continue;
                    }
    ...
    assert!(matches!(
        pg.activate_pricing_release_v2(&stale_target_head, &runtime_manifest)
            .unwrap(),
        crate::pricing::PricingReleaseActivationOutcomeV2::Rejected(
            crate::pricing::PricingReleaseActivationRejectionV2::CasMismatch { actual: Some(_) }
        )
    ));
    assert_eq!(
    ...
    let recovery = crate::pricing::PricingReleaseActivationRequestV2 {
        expectation: crate::pricing::PricingReleaseHeadExpectationV2::Exact(target_head.clone()),
        evidence: crate::pricing::PricingReleaseActivationEvidenceV2 {
...
fn postgres_stage8_consistent_rejection_evidence() {
    ...
    for (account_id, key_id, request_id, provider_id, offset, charged_hold, resolved, observed) in
        &scenarios
    {
