...
async fn canceled_sqlite_snapshot_reserve_never_reaches_durable_state() {
    ...
    assert!(
        registry::pricing::sqlite_legacy_scalar_admission_snapshot(&conn, "snapshot-canceled")
            .unwrap()
            .is_none()
    );
...
async fn lost_sqlite_snapshot_reserve_reply_stays_active_and_replays_exactly() {
    ...
    let conn = registry::open(&path_string).unwrap();
    assert_eq!(
        registry::pricing::sqlite_legacy_scalar_admission_snapshot(&conn, "snapshot-lost-reply")
            .unwrap(),
        Some(snapshot.clone())
    );
