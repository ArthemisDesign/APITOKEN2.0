...
pub fn cors() -> Response {
    ...
    h.insert("access-control-allow-headers",HeaderValue::from_static("Authorization, Content-Length, Content-Type, X-Goog-Api-Key, Idempotency-Key, X-Apitoken-Idempotency-Key, X-Goog-Upload-Protocol, X-Goog-Upload-Command, X-Goog-Upload-Offset, X-Goog-Upload-File-Name, X-Goog-Upload-Header-Content-Type, X-Goog-Upload-Header-Content-Length"));
    r
}
...
impl JsonlParser {
    fn push(&mut self, bytes: &[u8]) -> Result<(), &'static str> {
        for part in bytes.split_inclusive(|byte| *byte == b'\n') {
            if self.pending.len().saturating_add(part.len()) > JSONL_LINE_LIMIT {
...
pub async fn dispatch(
    ...
    match route {
        ...
        Route::Download(id) => file_download(facade, auth, id).await,
        Route::Upload => {
...
async fn create(
    ...
    let (reqs, input_kind, input_file_id) = match (inline_requests, file_name) {
        (Some(requests), None) => {
            ...
            (requests, registry::GeminiBatchInputKind::Inline, None)
        }
        (None, Some(file_name)) => {
            ...
            let requests = match file_requests(&f, &a.account, &file_id).await {
                Ok(requests) => requests,
                Err(response) => return response,
            };
            (
                requests,
                registry::GeminiBatchInputKind::File,
                Some(file_id),
            )
        }
        (Some(_), Some(_)) => {
    ...
    let mut total = 0i64;
    for (index, entry) in reqs.into_iter().enumerate() {
        let file_input = input_kind == registry::GeminiBatchInputKind::File;
        let (mut req, client_key, metadata) = if let Some(o) = entry.as_object() {
            (
                if file_input {
                    o.get("request")
                        .cloned()
                        .expect("JSONL parser requires request")
                } else {
                    o.get("request").cloned().unwrap_or(entry.clone())
                },
                o.get("key").and_then(Value::as_str).map(str::to_owned),
                (!file_input).then(|| o.get("metadata").cloned()).flatten(),
            )
        } else {
            return error(
                StatusCode::BAD_REQUEST,
        ...
        total = match total.checked_add(hold) {
            Some(total) => total,
            None => {
                return error(
                    StatusCode::BAD_REQUEST,
        ...
        let idx = match i64::try_from(index) {
            Ok(index) => index,
            Err(_) => {
                return error(
                    StatusCode::BAD_REQUEST,
        ...
        let request_id = fresh("batch-item");
        let plain = serde_json::to_vec(&req).unwrap();
        let blob = match f.keys().encrypt_blob(
        ...
        items.push(registry::GeminiBatchCreateItem {
            item_index: idx,
            request_id: request_id.clone(),
            logical_request_id: fresh("logical"),
            execution_group_id: fresh("exec"),
            client_key,
            request_digest: digest(&plain),
            input_file_id: None,
    ...
    let create = registry::GeminiBatchCreate {
        job_id: job.clone(),
        account_id: a.account.clone(),
        creator_key_id: a.key_id,
        public_model: model_id.clone(),
        display_name: display.clone(),
        canonical_request_digest: digest(&serde_json::to_vec(&json!({"model":model_id,"displayName":display,"priority":priority,"inputKind":if input_kind==registry::GeminiBatchInputKind::Inline{"inline"}else{"file"},"items":items.iter().map(|i|hex_digest(&i.request_digest)).collect::<Vec<_>>() })).unwrap()),
...
async fn file_upload(
    ...
    match upload.command {
        ...
        UploadCommand::Upload { finalize } => {
            ...
            if page.next_chunk_index.is_some() {
                return error(
                    StatusCode::FAILED_DEPENDENCY,
                    "FAILED_PRECONDITION",
                    "Upload has too many chunks for this subset.",
                );
            }
            let received = page
                .chunks
                .iter()
                .try_fold(0i64, |sum, chunk| sum.checked_add(chunk.plaintext_len))
                .unwrap_or(i64::MAX);
            if offset != received || received.saturating_add(raw.len() as i64) > file.size_bytes {
                return error(
                    StatusCode::CONFLICT,
                    "ABORTED",
            ...
            if !raw.is_empty() {
                let chunk = match f.keys().encrypt_file_chunk(
                    &super::GeminiBatchFileChunkIdentity {
                        account_id: &a.account,
                        file_id: &file_id,
                        chunk_index: page.chunks.len() as i64,
                        schema_version: SCHEMA_VERSION,
                    },
                    &raw,
                    now(),
                ...
                if !matches!(
                    f.authority()
                        .file_append(a.account.clone(), file_id.clone(), chunk)
                        .await,
                    Ok(true)
                ) {
                    return error(
            ...
            let new_offset = received.saturating_add(raw.len() as i64);
            if !finalize {
                let mut response = StatusCode::OK.into_response();
            ...
            let page = match f
                ...
                .await
            {
                Ok(page) if page.next_chunk_index.is_none() => page,
                _ => {
                    return error(
                        StatusCode::SERVICE_UNAVAILABLE,
                        "UNAVAILABLE",
            ...
            let completion = registry::GeminiBatchFileCompletion {
                ...
                chunk_manifest_digest: match super::gemini_batch_chunk_manifest_digest(&page.chunks)
                {
                    Ok(value) => value,
                    Err(_) => {
                        return error(
...
async fn create_file(
    ...
    if let Some(data) = data {
        ...
        for chunk in data.chunks(registry::MAX_BATCH_FILE_CHUNK_BYTES as usize) {
            ...
            if !matches!(
                f.authority()
                    .file_append(a.account.clone(), id.clone(), c)
                    .await,
                Ok(true)
            ) {
                return error(
...
async fn file_download(f: Arc<GeminiBatchPublicFacade>, a: MeteredAuth, id: String) -> Response {
    let file = match f.authority().file_get(a.account.clone(), id.clone()).await {
        Ok(Some(v)) if v.state == "active" => v,
        _ => {
            return error(
                StatusCode::NOT_FOUND,
                "NOT_FOUND",
    ...
    loop {
        let page = match f
            .authority()
            .file_chunks(a.account.clone(), id.clone(), after, 64)
            .await
        {
            Ok(v) => v,
            Err(_) => {
    ...
    let mut r = (StatusCode::OK, out).into_response();
...
mod tests {
    ...
    fn jsonl_parser_is_incremental_across_chunks_and_accepts_crlf() {
        let mut parser = JsonlParser::default();
        parser.push(b"{\"key\":\"a\",\"requ").unwrap();
        parser
            .push(b"est\":{\"contents\":[]}}\r\n\n{\"key\":\"b\",\"request\":{}}")
            .unwrap();
