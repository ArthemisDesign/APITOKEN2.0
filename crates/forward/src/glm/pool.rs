//! Attempt loop for the GLM (Zhipu AI / Z.ai) Coding Plan plane: the piece that turns known
//! profiles into served traffic.
//!
//! Contract: `docs/engine/PROVIDER_ONBOARDING.md` §8.1 and §8.4. This module is a pure
//! decision engine — it decides what to do after an attempt, and the caller performs the I/O.
//! Keeping it pure is what makes the one-byte boundary provable by test instead of by
//! inspection.
//!
//! Two rules everything else defers to:
//!
//! * **Once a single public byte has reached the client, no retry and no account switch may
//!   happen.** Replaying after that would either duplicate output or silently serve a
//!   different subscription mid-answer. It is encoded in the type system rather than as a
//!   check, so a caller cannot express the violation.
//! * **There is no auth retry.** Unlike KIMI the credential is a static key with no refresh
//!   family: a 401 means the key was revoked, and repeating the request with the same key is
//!   pointless. The first auth refusal is already terminal for the profile — the loop rotates
//!   away immediately instead of spending a same-profile retry.
...
pub enum ProfileEffect {
    None,
    /// Cool until the provider's window reset. Not a health failure — a capacity state.
    CoolUntilReset,
    /// The static key was refused or the plan expired: out of rotation until the Auth Bot
    /// publishes a replacement key.
    AccountDead,
    /// Risk-control fair-use or account anomaly: out of rotation pending review, recoverable.
    AccountSuspect,
    /// The requested model is outside this key's plan scope. Model-scoped, not account-wide.
    ModelIneligible,
...
pub fn decide(
    ...
    // The one-byte boundary. Everything below it is unreachable once delivery started: the
    // only honest option is to end the stream with the upstream's own error.
    ...
    match verdict {
        ...
        _ => {
            ...
            // Account/quota/model refusals do not spend budget, but they still need somewhere
            // to go. With no eligible profile left this is exhausted capacity, which is
            // retryable for the client rather than an error about their request.
...
fn effect_for(verdict: UpstreamVerdict) -> ProfileEffect {
    match verdict {
        ...
        UpstreamVerdict::ModelIneligible => ProfileEffect::ModelIneligible,
...
mod tests {
    ...
    #[test]
    fn a_first_auth_refusal_rotates_immediately_without_a_same_profile_retry() {
        // A static key cannot be refreshed: a repeated 401 is meaningless, so unlike KIMI
        // there is no RefreshAndRetrySameProfile step at all.
        let decision = decide(UpstreamVerdict::AccountDead, Delivery::PreByte, policy(), 5);
        assert_eq!(decision.next, NextStep::RotateToAnotherProfile);
        assert_eq!(decision.effect, ProfileEffect::AccountDead);
        // The refusal is the account's fault, not the upstream's: no transport budget burns.
    ...
    fn transport_faults_spend_the_budget_and_stop_at_zero() {
        ...
        assert_eq!(first.effect, ProfileEffect::TransportFault);
        assert_eq!(first.policy.transport_budget, 2);
        current = first.policy;

        let second = decide(UpstreamVerdict::Transport, Delivery::PreByte, current, 5);
        assert_eq!(second.policy.transport_budget, 1);
        current = second.policy;
    ...
    #[test]
    fn suspect_and_model_ineligible_rotate_with_their_own_effects() {
        // Suspect: out of rotation, but recoverable — never marked dead.
        let decision = decide(
            UpstreamVerdict::AccountSuspect,
            Delivery::PreByte,
            policy(),
        ...
        assert_eq!(decision.next, NextStep::RotateToAnotherProfile);
        assert_eq!(decision.effect, ProfileEffect::AccountSuspect);
        assert_ne!(decision.effect, ProfileEffect::AccountDead);

        // Model scope: the account keeps serving its other models.
        let decision = decide(
            UpstreamVerdict::ModelIneligible,
            Delivery::PreByte,
            policy(),
        ...
        assert_eq!(decision.next, NextStep::RotateToAnotherProfile);
        assert_eq!(decision.effect, ProfileEffect::ModelIneligible);
    }
