//! Bounded delivery queue between the GLM (Zhipu AI / Z.ai) stream finalizer and the
...
//! spend total that an earlier turn has not yet reached.** GLM turn events are dual-ledger
//! (official API nanoUSD AND native microcredits, `docs/engine/GLM_PROVIDER.md` §5.3), and
//! both ledgers advance per turn: if a turn's event is stuck and a later free quota poll went
//! ahead anyway, the estimator would see movement it cannot attribute on either ledger and
//! would either inflate capacity or record our own spend as somebody else's. So a failed head
//! stays at the front and blocks both later turns and quota polls until it drains.

use std::collections::VecDeque;

use registry::GlmTurnCalibrationEvent;
...
pub struct TurnQueue {
    queue: VecDeque<GlmTurnCalibrationEvent>,
    capacity: usize,
    dropped: u64,
...
impl TurnQueue {
    ...
    ///
    /// Overflow drops the NEWEST event, never the head. The head is what blocks quota polling,
    /// so discarding it to make room would release the block and let a poll pair quota with a
    /// spend total that never arrived — exactly the corruption this queue prevents.
    pub fn push(&mut self, event: GlmTurnCalibrationEvent) -> bool {
    ...
    pub fn head(&self) -> Option<&GlmTurnCalibrationEvent> {
        self.queue.front()
    }
    ...
    pub fn resolve_head(&mut self, outcome: WriteOutcome) {
        match outcome {
            ...
            WriteOutcome::Conflict => {
                ...
                // whole tail on one poisoned event would stall every subsequent turn
                // indefinitely.
                self.queue.pop_front();
                self.dropped = self.dropped.saturating_add(1);
                self.healthy = self.queue.is_empty();
    ...
    ///
    /// Reads never create evidence, but a read taken while a turn is undelivered would be
...
mod tests {
    ...
    fn event(id: &str) -> GlmTurnCalibrationEvent {
