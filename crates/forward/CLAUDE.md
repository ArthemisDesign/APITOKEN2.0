...
- **Circuit breaker (`breaker.rs`):** порог backend-фейлов в окне → размыкание; пока разомкнут,
  вход отбивает `503 + Retry-After` (не веерим по всему пулу при брауноуте api.anthropic.com).
- **Исчерпание пула:** пробовали, но все за лимитом → `429 + Retry-After = soonest_ready` (клиент
  откатится сам). Пул реально пуст → `503`. Каждый `mark_used` парен с `mark_ok`/`mark_healthy`+
  `end_stream`/`mark_cooling`/`mark_done`.
