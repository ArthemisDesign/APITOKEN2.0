...
- `api-limits` is the dependency-free checked payload contract. Current provider caps remain distinct:
  ...
  the shared bounded reader under the 256 MiB text envelope and the 20 MiB image/media cap. Native
...
hosted `web_search` records the declared client intent and is forwarded for execution; known
unexecutable hosted types (`file_search`, `computer*`, `mcp`) fail closed before admission.
A namespace contributes only its reviewed
...
3. **Model-visible context = explicit client base/developer instructions + replayed Responses
   ...
   `prompt_cache_options` are `400 documented_limitation` on that field (24h KV retention
   cannot be honoured on ChatGPT OAuth); `prompt_cache_key` stays accepted.
