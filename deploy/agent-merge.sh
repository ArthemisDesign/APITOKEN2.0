...
am_gate_typescript() (
  ...
  if [[ $mode != filtered ]]; then
    context_list=commerce,sales,openkeys,web,admin,devbot
  fi
...
am_gate_deployment() (
  ...
  bash "$ROOT/deploy/stage-unit-renderer.test.sh"
  bash "$ROOT/deploy/staging-foundation.test.sh"
...
am_gate_static() (
  ...
  python3 "$ROOT/deploy/repository-invariants.py"
