...
PLAN_DIGEST=sha256:v2:8e0d46b787b862e50044b1bd86a55ce1cbb37ef9facad438b5e3ba1db2a770e8
TARGET_GENERATION=21
TARGET_PLAN_DIGEST=sha256:v2:b6df45975bda85570f025c0687bafe77d23ba4774d10fd32cdf4ee513cab24e9
TARGET_RELEASE_DIGEST=sha256:v2:ff82cc44f43509e65b327a7ec2a508ed4c4461ad16659854c9de3d7c4c80b2ca
RECOVERY_GENERATION=22
RECOVERY_PLAN_DIGEST=sha256:v2:f5865a735fd250ddeae21e4919b3fd5f7aee5a3049f4b8008ab0de0351f37677
RECOVERY_RELEASE_DIGEST=sha256:v2:56787bab2bdae7425e1fc53294d7c6a157b91afc60f886795b64b1b54cf0ec7d
API_ORIGIN=http://127.0.0.1:8791
ACTOR=gpt-image-2-stage7-admission
REASON='align exact OpenKeys pricing lineages before GPT Image 2 cutover'
POLL_SECONDS=5
MAX_POLLS=360
STAGE56_PLAN=/var/lib/apitoken/pricing-stage56-admission/$ADMISSION_SHA/plan.json
STATE_PARENT=/var/lib/apitoken/pricing-stage7-admission
STATE_DIR=$STATE_PARENT/$ADMISSION_SHA
REQUEST_STATE=$STATE_DIR/request.json

[[ $# -eq 1 ]] || { printf 'usage: pricing-stage7-admission-gate.sh <exact-admission-sha>\n' >&2; exit 2; }
[[ $1 == "$ADMISSION_SHA" ]] \
  || { printf 'pricing Stage 7 admission is pinned to %s\n' "$ADMISSION_SHA" >&2; exit 1; }
[[ $(id -u) == 0 ]] \
  || { printf 'pricing Stage 7 admission must run through the fixed root bridge\n' >&2; exit 1; }
command -v curl >/dev/null || { printf 'curl is required\n' >&2; exit 1; }
command -v jq >/dev/null || { printf 'jq is required\n' >&2; exit 1; }
command -v flock >/dev/null || { printf 'flock is required\n' >&2; exit 1; }

work=$(mktemp -d /run/pricing-stage7-admission.XXXXXX)
chmod 0700 "$work"
