...
EVIDENCE_PARENT=$STATE_ROOT/gpt-image-2-public-preflight-v3
OUTPUT=$EVIDENCE_PARENT/$PRODUCER_SHA

[[ $# -eq 1 ]] || wd_die "usage: gpt-image-2-public-preflight-v3-gate.sh <exact-producer-sha>"
SHA=$1
wd_validate_sha "$SHA"
[[ $SHA == "$PRODUCER_SHA" ]] \
  || wd_die "GPT Image 2 public preflight v3 is pinned to producer $PRODUCER_SHA"
[[ $(id -u) == 0 ]] || wd_die "GPT Image 2 public preflight v3 must run through the fixed root bridge"
command -v jq >/dev/null || wd_die "jq is required for GPT Image 2 public preflight v3 evidence"
...
journal_summary() {
  ...
  jq -er --arg sha "$PRODUCER_SHA" '
    ...
    if $valid then "gpt-image-preflight-v3:\($journal.state):g=false:e=false"
...
verify_preflight_success() {
  ...
  [[ $summary == gpt-image-preflight-v3:preflight_success:g=false:e=false ]]
}

# A SHA-keyed root is a one-shot fence even though this mode cannot dispatch an image.
if [[ -e $OUTPUT || -L $OUTPUT ]]; then
  verify_preflight_success \
    || wd_die "prior GPT Image 2 public preflight v3 is fenced without exact success evidence"
...
if [[ -e $EVIDENCE_PARENT || -L $EVIDENCE_PARENT ]]; then
  path_is_private_deploy_directory "$EVIDENCE_PARENT" \
    || wd_die "GPT Image 2 public preflight v3 parent is not an actual private deploy directory"
else
  install -d -o deploy -g deploy -m 0700 -- "$EVIDENCE_PARENT"
fi
path_is_private_deploy_directory "$EVIDENCE_PARENT" \
  || wd_die "GPT Image 2 public preflight v3 parent is not private"

release=$ENGINE_RELEASE_ROOT/$PRODUCER_SHA
current=$(readlink -f -- "$ENGINE_RELEASE_ROOT/current")
[[ $current == "$release" ]] \
  || wd_die "GPT Image 2 public preflight v3 requires current producer release $PRODUCER_SHA"
binary=$release/claude-api
[[ -f $binary && ! -L $binary && -x $binary ]] \
  || wd_die "exact GPT Image 2 public preflight v3 binary is missing"
