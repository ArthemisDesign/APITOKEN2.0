...
case "${1:-}" in
  ...
  deployment-create)
    ...
    [[ $2 =~ $sha_re ]] || { echo 'invalid SHA' >&2; exit 2; }
    [[ $3 =~ ^production-(database|engine|backend|sales|openkeys|admin)$ ]] || { echo 'invalid environment' >&2; exit 2; }
  ...
  deployment-status)
    ...
    [[ $5 =~ ^(candidate-validation|production-(database|engine|backend|sales|openkeys|admin))$ ]] || { echo 'invalid environment' >&2; exit 2; }
