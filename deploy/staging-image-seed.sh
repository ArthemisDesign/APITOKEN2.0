...
seed() {
  local source=$1 source_tag=$2 target=$3 archive
  ...
  docker image inspect "$source_tag" >/dev/null
  ...
  runuser -u "$STAGE_USER" -- env DOCKER_HOST="unix://$SOCKET" docker image inspect "$source_tag" >/dev/null
...
seed \
  ...
  postgres:18-alpine apitoken-stage/postgres:18-alpine
seed \
  redis:7.4.2-alpine@sha256:02419de7eddf55aa5bcf49efb74e88fa8d931b4d77c07eff8a6b2144472b6952 \
  redis:7.4.2-alpine apitoken-stage/redis:7.4.2-alpine
