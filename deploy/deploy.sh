...
validate_engine_stage() {
  ...
  [[ -x "$directory/claude-router" ]] || die "staged router binary is missing: $directory/claude-router"
...
prepare_engine_release() {
  ...
  if [[ -n "$TESTED_CANDIDATE" ]]; then
    ...
    run install -m 0755 -- "$TESTED_CANDIDATE/.deploy-artifacts/engine/claude-router" \
      "$ENGINE_STAGE/claude-router"
  else
    prepare_engine_source
...
if [[ -n "$TESTED_CANDIDATE" ]]; then
  validate_tested_candidate "$TESTED_CANDIDATE" "$TESTED_MARKER" "$SHA" \
    "$DEPLOY_API" "$DEPLOY_ENGINE" commerce
else
  fetch_release_commit
fi
