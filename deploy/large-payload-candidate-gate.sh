#!/usr/bin/env bash
set -euo pipefail
usage() { echo 'usage: large-payload-candidate-gate.sh <sha> <url> <unit> <spool-root> <memory-high-bytes> <evidence-dir>' >&2; exit 2; }
[[ $# == 6 ]] || usage
sha=$1 url=$2 unit=$3 spool=$4 memory_high=$5 evidence_dir=$6
...
python3 "$script_dir/tests/large_payload_mock_load.py" --url "$url" --sizes-mib "8,32,64" --concurrency 4 >"$load"
