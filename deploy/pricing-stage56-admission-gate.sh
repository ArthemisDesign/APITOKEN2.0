...
ACTOR=gpt-image-2-stage56-admission
REASON='prepare GPT Image 2 generation 6 customer pricing admission'
POLL_SECONDS=5
MAX_POLLS=360
STATE_PARENT=/var/lib/apitoken/pricing-stage56-admission
STATE_DIR=$STATE_PARENT/$ADMISSION_SHA
PLAN_STATE=$STATE_DIR/plan.json

[[ $# -eq 1 ]] || { printf 'usage: pricing-stage56-admission-gate.sh <exact-admission-sha>\n' >&2; exit 2; }
[[ $1 == "$ADMISSION_SHA" ]] \
  || { printf 'pricing Stage 5/6 admission is pinned to %s\n' "$ADMISSION_SHA" >&2; exit 1; }
[[ $(id -u) == 0 ]] \
  || { printf 'pricing Stage 5/6 admission must run through the fixed root bridge\n' >&2; exit 1; }
command -v curl >/dev/null || { printf 'curl is required\n' >&2; exit 1; }
command -v jq >/dev/null || { printf 'jq is required\n' >&2; exit 1; }

work=$(mktemp -d /run/pricing-stage56-admission.XXXXXX)
chmod 0700 "$work"
trap 'rm -rf -- "$work"' EXIT

[[ ! -L $STATE_PARENT && ! -L $STATE_DIR ]] \
  || { printf 'pricing Stage 5/6 state path must not be a symlink\n' >&2; exit 1; }
...
[[ $(stat -c '%U:%G:%a' -- "$STATE_PARENT") == root:root:700 \
  ...
  || { printf 'pricing Stage 5/6 state directories are not private root authority\n' >&2; exit 1; }
exec 9>"$STATE_DIR/lock"
chmod 0600 "$STATE_DIR/lock"
flock -n 9 || { printf 'another pricing Stage 5/6 admission is active\n' >&2; exit 1; }
