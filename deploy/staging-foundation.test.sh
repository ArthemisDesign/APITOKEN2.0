...
grep -Fq "docker image inspect --format '{{index .RepoTags 0}}'" "$ROOT/deploy/staging-image-seed.sh"
...
grep -Fq "docker image inspect --format '{{.Id}}'" "$ROOT/deploy/staging-image-seed.sh"
grep -Fq 'docker tag "$source_id" "$image"' "$ROOT/deploy/staging-image-seed.sh"
