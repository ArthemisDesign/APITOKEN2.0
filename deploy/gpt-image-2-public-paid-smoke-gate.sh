...
[[ $# -eq 1 ]] || wd_die "usage: gpt-image-2-public-paid-smoke-gate.sh <exact-producer-sha>"
SHA=$1
wd_validate_sha "$SHA"
[[ $SHA == "$PRODUCER_SHA" ]] \
  || wd_die "GPT Image 2 public paid smoke is pinned to producer $PRODUCER_SHA"
...
if ! timeout --signal=TERM --kill-after=15s 1500s \
