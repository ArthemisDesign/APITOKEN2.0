...
PRODUCER_SHA=853fdc6c8d5be486c371b23df6772eeaf7a48029
EVIDENCE_PARENT=$STATE_ROOT/gpt-image-2-public-paid-smoke-v2
OUTPUT=$EVIDENCE_PARENT/$PRODUCER_SHA

[[ $# -eq 1 ]] || wd_die "usage: gpt-image-2-public-paid-smoke-v2-gate.sh <exact-producer-sha>"
SHA=$1
wd_validate_sha "$SHA"
[[ $SHA == "$PRODUCER_SHA" ]] \
  || wd_die "GPT Image 2 public paid smoke v2 is pinned to producer $PRODUCER_SHA"
