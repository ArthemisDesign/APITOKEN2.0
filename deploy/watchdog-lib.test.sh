...
grep -Fq 'CLAUDE_API_PROVIDER=anthropic CLAUDE_API_CLAUDESTORE_CODEX_FALLBACK_ENABLED=0 CLAUDE_API_GLM_ENABLED=0 CLAUDE_API_KIMI_ENABLED=0 CLAUDE_API_TRUST_LOOPBACK=0 CLAUDE_API_HOST=127.0.0.1' \
  "$ROOT/systemd/claude-api-anthropic@.service"
...
# permission) and a separate unit change after live evidence. KIMI production is origin 8803;
# the same units pin it off so a shared env cannot re-embed that gateway on Claude slots.
for anthropic_serving_unit in claude-api.service claude-api@.service \
  ...
  grep -Fq 'CLAUDE_API_KIMI_ENABLED=0' "$ROOT/systemd/$anthropic_serving_unit" \
    || wd_die "$anthropic_serving_unit can inherit the dedicated KIMI plane from shared env"
  ! grep -Fq 'CLAUDE_API_KIMI_ENABLED=1' "$ROOT/systemd/$anthropic_serving_unit" \
    || wd_die "$anthropic_serving_unit still argv-enables the KIMI embed"
done
...
grep -Fq 'job!~"claude-router|devbot|staging-veth|.*-http"' "$ROOT/deploy/watchdog.sh" \
  || wd_die "final monitoring verify must exclude probe jobs and staging from exporter up"
grep -Fq 'query=up{$up_filter} == 0' "$ROOT/deploy/watchdog.sh" \
  || wd_die "final monitoring verify must list the down jobs on failure"
