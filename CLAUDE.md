...
прогоняет полный gate, берёт машинный merge-lock, ребейзит, перепроверяет gate на ТОМ САМОМ SHA,
который пушит, и держит lock до зелёного `deploy/watchdog`. Мёржить в `master` вслепую или вручную
запрещено: `master` — production trigger, и watchdog деплоит ровно один SHA за раз.
