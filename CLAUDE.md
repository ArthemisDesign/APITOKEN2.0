...
Скрипт сначала и повторно под lock сам проверяет именно `deploy/watchdog` для текущего `master`,
переиспользуя credential, которым `git` уже пушит (на macOS — Keychain); отдельные `gh` и
`GITHUB_TOKEN` не нужны. Pending или временно недоступный status он ждёт и перепроверяет сам, до
зелёного предыдущего SHA полный gate не запускает. Агент НИКОГДА не просит человека дать токен или
доказать зелёный деплой: чинит локальный credential helper и перезапускает команду. Затем скрипт
параллельно прогоняет локальный полный gate и trusted validation точного запушенного SHA на
production-хосте, берёт машинный merge-lock и ребейзит. Оба результата переиспользуются только если
SHA не изменился; иначе feature-ветка обновляется и оба gate повторяются для нового SHA. После push
хост переиспользует тот же замороженный кандидат без повторного теста, а скрипт держит lock до
