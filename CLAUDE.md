...
параллельно прогоняет локальный полный gate и trusted validation точного ребейзнутого и
запушенного SHA на production-хосте, затем берёт машинный merge-lock. Оба результата
переиспользуются только если SHA не изменился; иначе feature-ветка обновляется и оба gate
повторяются для нового SHA. После push хост переиспользует тот же замороженный кандидат без
