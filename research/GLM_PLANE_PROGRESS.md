...
| Observability: operational status, admin-only `GET /glm-subs` (+fleet `window_totals`), aggregate метрики, `glm-provider` алерты + runbook + consistency-пины | `crates/{forward,server}`, `observability/**`, `deploy/**`, `docs/**` | 8f580619 |
| Безопасный live-runner + offline tests + ops runbook | `tools/glm_calibration/{run_live,test_run_live}.py`, `docs/ops/GLM_CALIBRATION.md`, `docs/README.md`, §8 манифеста | 1d1123c2 |
| Фингерпринт: полный Claude Code отпечаток | `crates/forward/src/glm/{transport,gateway}.rs`, `crates/server/src/config.rs` | 764deb39 |
| Admin UI consumer: GLM capacity board на `/subscriptions` (same-origin `/glm-subs`) | `apps/admin`, `docs/product/ADMIN_PANEL.md`, `docs/engine/GLM_PROVIDER.md` | _этот коммит_ |
