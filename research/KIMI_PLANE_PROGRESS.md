...
**Процессная заметка:** не редактировать worktree, пока в нём идёт `agent-merge.sh` — дерево
меняется под гейтом, и он падает на несобранном срезе. Один мёрж уже потерян так.
