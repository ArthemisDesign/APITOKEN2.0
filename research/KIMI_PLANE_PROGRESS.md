...
всех `kimi/*` и дополнительно `video` для семейства k3 (`k3`, `k3[1m]`, `k3-256k`) — video
задекларировано официальной страницей Kimi Code только для него; coding-алиасы остаются
image-only. Tool-проба остаётся открытой ногой: provider-executed tools по-прежнему отклоняются
