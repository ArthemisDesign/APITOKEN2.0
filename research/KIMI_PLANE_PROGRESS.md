...
**Не проверено:** каденция health-свипа против уроков Codex, shutdown-барьер, границы тела и
неподдерживаемых поверхностей.
...
**Не сделано намеренно (решение владельца 2026-08-07):** публикация в каталог роутера и
`routing-presets.json`, а также авторитет цен release-v2. Kimi резервирует легаси-скаляром
(`reserve_request_for_execution` × `mult_bp`), и это доказано точным до наноцента списанием на
живых прогонах. Гейт `pricing-catalog-coverage` требует записи в каталоге цен только для
**рекламируемых** моделей — мы ничего не рекламируем, поэтому Kimi доступна по имени с бекенда и
отсутствует в `/v1/models` и на сайте. Как только решим публиковать — сначала `SnapshotProvider::Kimi`
и записи каталога, только потом пресет.
