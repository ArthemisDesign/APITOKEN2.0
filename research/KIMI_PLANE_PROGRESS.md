...
**2026-08-17 — media-проба прогнана live, `image_input` открыт.** Отдельный media-only прогон
(coverage-плечи были уже покрыты ранее; диспатчились только пробные ноги, чтобы агрегатный
потолок покрывал ровно доказываемое) на профиле `kimi-3e80dc1f94df83de` (Vivace): 1×1 PNG
inline-part принят всеми четырьмя ногами — `k3-256k:256k:low` ($0.000897), `kimi-for-coding:256k:high`
($0.000327), `kimi-for-coding-highspeed:256k:high` ($0.000379), `kimi-for-coding:256k:off`
($0.000204). Каждая нога атрибутирована exact request-id к неизменяемому событию, тарифные пины
`moonshot/kimi/<модель>/v2`, стоимость на три порядка ниже worst-case bound. Итоговый расход
$0.0018 из авторизованных $1.80. Каталог-производитель теперь публикует `image_input: true`
(`crates/server/src/router_catalog.rs`), роутер отдаёт `input_modalities:["text","image"]` для
всех `kimi/*`. Первичная video-матрица была записана неверно и перепроверена 2026-08-18:
официальная таблица Kimi Code гарантирует image+video для `k3`, `kimi-for-coding` и
`kimi-for-coding-highspeed`, но только image для `k3-256k`; локальный `k3[1m]` наследует wire
model `k3`. Минимальный 0.12-секундный 16x16 MP4 live принят и правильно распознан как чёрный
всеми тремя гарантированными id: exact request-id attribution, 2xx, стоимость $0.001302 для k3,
$0.000217 для coding и $0.000547 для highspeed. `k3-256k` сейчас тоже принял MP4 ($0.001287),
но каталог намеренно не рекламирует undocumented возможность вопреки официальному image-only
контракту. Tool-проба остаётся открытой ногой: provider-executed tools по-прежнему отклоняются
с `kimi_tools_unpriced` до доказательства их удельной стоимости.
...
**2026-08-10 — strict-блокер закрыт не нами.** Ночной демонтаж прайсинга («one price, one
settlement, no retired pricing left», коммиты `9f3f65d0`, `ca3dd6c9`, `0c236aa2`, `a6e7f2e7`) убрал
понятие strict-политики целиком: `strict_policy` больше нет ни в `crates/forward`, ни в
`crates/server`, вместе с ним ушёл и отказ в `KimiGateway::handle`. Проверено живым клиентским
ключом: `kimi/k3` и `kimi/kimi-for-coding-highspeed` обслужены.

Вывод для журнала: анализ 2026-08-09 был верен про механизм (strict-писатель требовал
`policy_v1`-снимок, который умеет строить только каталожная политика) и неверен про лекарство —
не хватало не каталожной записи для `kimi`, а завершения демонтажа этажом выше. Задача №17
(«завести каталожную запись») закрыта как неактуальная.
