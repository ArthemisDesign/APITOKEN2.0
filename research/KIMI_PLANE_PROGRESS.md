...
**Остаётся открытым: авторитет цен release-v2.** `SnapshotProvider` знает только
`anthropic`/`openai`/`google`. Следствие: шлюз **отказывает strict-ключу** сразу
(`kimi_strict_pricing_unavailable`), а `router_pricing.rs` для такого ключа выбрасывает все
`kimi/*` кандидаты — то есть strict-ключ вообще не видит Kimi в `/v1/models`. Поверхности
согласованы и клиент не получает загадочный 529, но это ровно то же положение, в котором
находится Gemini. Публикацию это не блокирует: после ратификации генерации 55 как последней
новые модели идут через `is_model_unpriced` → точный легаси-тариф + горячий seed — ровно тот
путь, которым Kimi и резервирует. Гейт `pricing-catalog-coverage` удалён на master в `b4a694c3`
по той же причине, так что сверять публикацию с каталогом релиза больше нечем и не нужно.
