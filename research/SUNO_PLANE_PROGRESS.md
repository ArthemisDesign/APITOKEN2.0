...
Server wiring (`crates/server/src/{config,main,poller,http}.rs`, separate commit): Suno plane
composition (`CLAUDE_API_SUNO_*`, `CLAUDE_API_PROVIDER=suno`), the `GET /suno-subs` admin
projection (privacy-safe, calibration joined via `profile_id_for_subject`), `claude_api_suno_*`
metrics and the `suno-provider` alert group + MONITORING.md sections. The runtime plane
(forward) is delivered.
