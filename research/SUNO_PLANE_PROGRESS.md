...
Generation gateway (`crates/forward/src/suno/{gateway,session,artifacts,upload}.rs`, separate
commit): per-profile JWT-mint/`set-cookie` re-seal single-flight, reserve → hCaptcha pre-check →
create → detached poll → download → settle (attributed delta, else documented reserve), FIFO →
registry writer with quota observation pairing, then server wiring (`crates/server`) + admin
projection + observability. The runtime primitives and migration 0052 are delivered.
