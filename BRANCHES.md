...
3. **`master` = production trigger.** Мёрж только через `deploy/agent-merge.sh` и только когда
   изменение полностью готово к production. Скрипт сериализует мёржи машинным локом, поэтому два
   кандидата никогда не тестируются и не деплоятся друг поверх друга. Host watchdog прогонит
   полный gate, migration-before-app и blue-green deploy; итог виден в `deploy/watchdog`.
