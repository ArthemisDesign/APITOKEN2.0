...
3. **`master` = production trigger.** Мёрж только через `deploy/agent-merge.sh` и только когда
   ...
   затем параллельно запускает локальный полный gate и trusted host-validation точного feature SHA,
   переиспользуя credential из `git credential` и не перекладывая токен/доказательство на человека.
   После ребейза оба результата действуют только для неизменившегося SHA. Скрипт сериализует мёржи
   машинным локом, поэтому два production-кандидата никогда не деплоятся друг поверх друга. Тот же
   замороженный host-кандидат переиспользуется после push в `master`; затем watchdog выполняет
   migration-before-app и blue-green deploy, а итог виден в `deploy/watchdog`.
