...
3. **`master` = production trigger.** Мёрж только через `deploy/agent-merge.sh` и только когда
   ...
   ветку на последний закоммиченный `master`, затем параллельно запускает локальный полный gate и
   trusted host-validation точного feature SHA, переиспользуя credential из `git credential` и не
   перекладывая токен/доказательство на человека. Pending target может перекрываться с этими
