...
def send(url, size, chunked, timeout):
    ...
    headers = {"content-type": "application/json", "authorization": "Bearer load-gate"}
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as pool:
        rows=list(pool.map(lambda item: send(args.url,item[0],item[1],args.timeout),work))
