...
- Administrators work the queue in **Admin → Partners → Access applications**
  ...
  instead of marking an account approved that never got access. A successful applicant submit
  fail-open notifies the Devbot 🤝 Partners topic when `DEVBOT_PARTNER_WEBHOOK_URL` is set; an
  administrator decision does not. Rejecting records the note and never
