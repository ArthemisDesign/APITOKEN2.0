...
Все шаги §6 плана, относящиеся к первой волне, выполнены (вторая волна — R1 на Claude/Gemini и
R5a — по решению самого плана запускается только по метрикам из шага 0 после продакшн-наблюдения):

| Шаг | Коммит | Статус |
|---|---|---|
| Журнал + индекс docs | `0f669924` | ✅ |
| 0. Наблюдаемость (provider_sub_snapshots, 4 плоскости) | `3afd8c6f` | ✅ |
| R2. Claude placement: min(free/cap) − 0.5·util7 | `05e42e51` | ✅ |
| R7. Kimi + GLM: окна раздельно, weekly-ключ, дисконт 5h, резерв 95% | `ef47a978` | ✅ |
| R1 v1. Codex: time-decay джиттерованного порога + pace guard | `a5a1e04d` | ✅ |

Финальная верификация на ветке `feat/quota-5h7h-split-impl`:
- `cargo test --workspace` — все зелёные (forward 1544, pool 59, claude-api 195 и т.д.);
- `cargo build` — зелёный;
- `deploy/docs-check.sh origin/master..HEAD` и `git diff --check` — чисто;
- ветка запушена (`git push -u origin HEAD`).

Контроль после выката (§6): вариация util7d/tightest fraction по флоту должна снизиться; доля
домов на 7d-резерве до конца недели → ~0 до последних часов; `route_rebind/(pin+place)` и
cache-hit НЕ должны измениться (дрейф = регрессия); частота провайдерских 429 не должна вырасти.
Ряды для этого уже пишутся: `provider_sub_snapshots` (metrics.db) + `Pool::route_stats` (/metrics).
