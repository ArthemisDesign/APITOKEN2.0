...
| admin UI consumer | `apps/admin` | готово: same-origin потребитель `/glm-subs` — GLM capacity board на `/subscriptions` (fleet-карточка, окна по аккаунтам, exact API-$, native microcredits) |
