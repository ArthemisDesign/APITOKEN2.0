...
alternate value is `systemd-flat`, retained for an explicitly operator-launched, non-public
exact-SHA calibration process: systemd may import a private roster credential plus the source
credential directory as read-only files named `gemini-credential_<profile-id>.json`, and the loader
still requires the exact derived path with no fallback to the normal tree. No production unit uses
or provisions this alternate layout.
...
The ordinary final-turn gate requires non-empty user text and rejects prefilled model and
image-only final turns. A final user turn whose
parts are exclusively `functionResponse` objects — the transcript every portable tool-calling
client produces after executing a call — is admitted for all traffic: the closed
`--gemini-37-tool-result-final-turn` runner mode proved that wire contract live (run
gemini-cal-1787152582-af5e9cfb, request fa042530-3c7d-4aff-b795-ea1c3b2b0122: upstream
`gemini-3.7-flash-tiered` returned incremental SSE with visible text, terminal `STOP` and
authoritative terminal usage).
...
- surfaces a mid-stream upstream error as a sanitized native error element rather than a clean
  truncation;
- caps text requests at the current 32 MiB transport envelope, documented inline-media requests at
  20 MiB, translated non-stream responses at 32 MiB, and generated-image response bodies/pending
  stream frames at 64 MiB. These distinct current values are pinned in `crates/api-limits`; its
  future 256 MiB hard ceiling is not enablement before binary IPC and weighted admission. Published
...
`candidates[].content.parts[].inlineData`; images are never written to server disk. Inline
`application/pdf` parts are admitted as document input (the 2026-08-16 fleet media matrix proved
beacon extraction with terminal usage/parity on this route); audio and video are not part of the
model's official input surface and stay rejected.

The canonical OpenCode integration deliberately does not advertise this as image output. Itscustom provider is `@ai-sdk/openai-compatible` 2.0.41, whose Chat response schema accepts message
...
|---|---|---|---|
| `gemini-3.7-flash` | public → `gemini-3.7-flash-tiered`; the same exact private row is the sole quota identity; ordinary responses rewrite it to the public id | Google announced the Developer API model GA on 2026-08-13; exact-public-ID runtime `20d945ce…` was withdrawn after an attested count plus paid SSE 404. Tiered runtime `2c8aca0d…` was withdrawn after a positive count and reconciled `MAX_TOKENS`. Production-GREEN successor `c4f0773a…` used explicit 512 and passed one count plus one no-retry paid generation: exact text, 8/7 incremental frames, terminal `STOP`/usage, raw confirmed tiered alias, 20 input + 478 output tokens and reconciled `$0.0018075`. The 2026-08-15 per-level matrix on `916dee0d…` then admitted explicit `low`/`medium`/`high` with exact output, terminal STOP/usage and positive thinking classes (`$0.00546` aggregate). The same-day capability matrix on `35153abe…` admitted forced function calling, schema-valid JSON structured output, inline image input and implicit caching with 8,170 authoritative cached tokens (`$0.0190` aggregate); a one-query search admission on `ed63dc0f…` added grounding with the exact per-query SKU reconciled (`$0.014921`); the 2026-08-16 media matrix on `fc556402…` admitted inline audio (WAV tone marker), video (MP4 color answer) and PDF (beacon extraction) inputs with mandatory content-perception markers (`$0.00119025` aggregate) | published for text+image+audio+video+pdf input, default and explicit thinking levels `low`/`medium`/`high`, function calling, JSON structured output, implicit caching and Google Search grounding |
| `gemini-3-flash-preview` | public → `gemini-3-flash`; quota admission joins `gemini-3-flash` + `gemini-3-flash-agent`; configured Antigravity origin, 2.2.1 UA, minimal headers; bounded inline PCM WAV fallback uses exact integral `duration × 32` AUDIO tokens and fails closed on ambiguous cache | fresh runner SHA `cc7e5beb…` / byte-identical runtime implementation completed 22 paid turns on Pro+Ultra: minimal/low/medium/high, incremental SSE, final cache reads with 8,170 cached tokens, fresh/replayed 8-token PCM audio and forced function calls; public identity and terminal response/event usage matched; 2026-08-16 fleet media matrix admitted video and pdf with perception markers | published, including audio/video/pdf input; generation 5 main catalog, production defaults, router manifest and public web/docs |
| `gemini-3.6-flash` | low → `gemini-3.6-flash-low`; medium/default → `gemini-3.6-flash-medium`; high → `gemini-3.6-flash-high` | default/minimal/low/medium/high: generate 200, incremental SSE 200, countTokens 200; canonical modelVersion and non-zero usage verified on 2026-07-31; 2026-08-16 fleet media matrix admitted audio/video/pdf with content-perception markers and terminal usage/parity | published, including audio/video/pdf input |
| `gemini-3.5-flash` | minimal → `gemini-3.5-flash-extra-low`; low/medium/high/default → `gemini-3.5-flash-low`, with the requested native thinking level preserved | default/minimal/low/medium/high: generate 200, incremental SSE 200, countTokens 200; default and `alt=json` JSON streams 200; canonical modelVersion and non-zero usage verified on Google AI Pro on 2026-07-31; 2026-08-16 fleet media matrix admitted audio/video/pdf | published, including audio/video/pdf input |
| `gemini-3.1-pro-preview` | low → `gemini-3.1-pro-low`; medium/high/default → `gemini-pro-agent` with the requested native thinking level preserved | default/low/medium/high: generate 200, incremental SSE 200, countTokens 200; canonical modelVersion and non-zero usage verified on 2026-07-31; 2026-08-16 fleet media matrix admitted audio/video/pdf | published, including audio/video/pdf input |
| `gemini-3.1-flash-lite` | same id | generate 200, SSE 200; 2026-08-16 fleet media matrix admitted audio/video/pdf | published, including audio/video/pdf input |
| `gemini-2.5-flash` | same id | generate 200, SSE 200; 2026-08-16 fleet media matrix admitted audio and video with perception markers; pdf has no official claim, and the audio leg carries no separate upstream AUDIO token class on this route | published for text/image/audio/video input |
| `gemini-2.5-flash-lite` | same id | generate 200, SSE 200; 2026-08-16 fleet media matrix admitted video and pdf with perception markers; the audio leg carries no separate upstream AUDIO token class on this route | published for text/image/video/pdf input |
...
scheduler permits at most two simultaneously executing Batch items per active subscription; its
separate global worker bound remains four, and ordinary synchronous Gemini traffic is unchanged.
