...
3. **No shared execution queues, semaphores, or circuit breakers in the router.**
   ...
   conjunction of all planes' health; there are no synchronous health checks on the
    request path. The single exception is the fail-fast 4 GiB estimated-RSS and 16 GiB spool
    admission for materialized universal request bodies: a known size is rounded up in 1 MiB steps;
    ...
    120-second idle and a 30-minute absolute body-read deadline, and does not hold the
...
right to materialize up to 256 MiB from an unauthenticated client. The producer-first
...
After auth the consumer makes fail-fast reservations against independent 4 GiB estimated-RSS and
16 GiB spool budgets in 1 MiB steps. Both are owned by `crates/bounded-body` RAII reservations;
...
`CLAUDE_ROUTER_BODY_SPOOL_ROOT`; each systemd slot owns a separate mode-0700 StateDirectory under
`/var/lib/apitoken/spool` and the path is never logged. The memory threshold is 8 MiB, so bodies
above that spill to the disk-backed private file. Request `Content-Encoding` other than `identity` is
...
(`max_size 256MiB`, no buffering / no `flush_interval`). Namespaced single-model JSON is routed by an incremental top-level selector
...
Malformed, zero, inconsistent, or above-current values fail startup. A valid
...
request must not land on the Claude slots. The router is a byte-faithful proxy of the
client path, so Chat (`/v1/chat/completions`) and Responses (`/v1/responses`) arrive at
8803 unchanged. The dedicated KIMI plane therefore mounts the same Anthropic-plane
adapters; a plane that only served `/v1/messages` 404'd those clients
(`unsupported_endpoint`) after the Anthropic embed was pinned off. Anthropic and combined
rollback units pin `CLAUDE_API_KIMI_ENABLED=0`; only origin 8803 composes the gateway.
...
3. **Universal Chat (2–4 weeks).** `/v1/chat/completions` for all catalog models:
   ...
   (`crates/router/src/chat.rs`) buffers only the request body (256 MiB — the current
...
4. **Universal Responses for Codex parity (2–4 weeks).** `POST /v1/responses` for all
   ...
   only the request body is buffered (256 MiB), `model` is extracted, the plane is
...
5. **Anthropic Skin for non-Claude models (3–5 weeks).** A Messages entry for
   ...
   body is buffered (256 MiB), the `openai/` namespace prefix selects the Codex plane
