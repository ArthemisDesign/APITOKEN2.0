...
| router publication | `crates/server/src/router_catalog.rs`, `crates/router/src/{catalog,policy,routing}.rs` | done: internal discovery producer, fourth optional catalog plane over the Anthropic lane, `kimi` arm in the router pricing producer, five aliases advertised |
