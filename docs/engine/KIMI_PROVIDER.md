...
| blue-green | `systemd/claude-api-kimi{,@}.service`, `deploy/**`, `observability/prometheus/prometheus.yml` | done: two slot units 8804/8805 + stable loopback origin 8803, capability marker, rollback branch stops all incarnations, scrape target `provider: kimi`, `ProviderMode::Kimi` with fail-closed non-KIMI `/v1/messages` plus Anthropic-plane Chat/Responses adapters; plane **enabled** by the reviewed argv pin `CLAUDE_API_KIMI_ENABLED=1` after live evidence 2026-08-04 |
...
| router publication | `crates/server/src/router_catalog.rs`, `crates/router/src/{catalog,policy,pricing,routing}.rs`, `systemd/claude-router{,@}.service` | done: internal discovery producer, fourth optional catalog plane, Anthropic Messages lane with dedicated origin 8803 (`CLAUDE_ROUTER_KIMI_ORIGIN`) that also mounts Chat/Responses adapters because the router forwards those paths unchanged, `kimi` arm in pricing/policy origin order, five aliases advertised |
