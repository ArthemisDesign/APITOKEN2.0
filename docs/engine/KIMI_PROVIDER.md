...
| router publication | `crates/server/src/router_catalog.rs`, `crates/router/src/{catalog,policy,pricing,routing}.rs`, `systemd/claude-router{,@}.service` | done: internal discovery producer, fourth optional catalog plane, Anthropic Messages lane with dedicated origin 8803 (`CLAUDE_ROUTER_KIMI_ORIGIN`), `kimi` arm in pricing/policy origin order, five aliases advertised |
