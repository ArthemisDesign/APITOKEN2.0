...
`decision` По §10.1 KIMI — **GPT-подобный dual-ledger** провайдер, но с оговоркой:

1. **API-nanoUSD ledger** — точный, из официального rate card §5.1 по served-модели.
2. **Native quota ledger** — целые `used`-единицы по каждому окну, **семантика не доказана**.
3. **Booster wallet** — реальные деньги, отдельный ledger.

Ни один не восстанавливается из другого. Пока единица `used` не доказана live-прогоном,
`native_capacity` публикуется, а её интерпретация помечается незрелой; `capacity_nanoUSD`
считается по формуле §10.5 от доли, полученной как `used/limit`.
