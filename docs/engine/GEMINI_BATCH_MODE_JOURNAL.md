...
## 2026-08-21 — Этап 6 (§6): pre-activation publication entry
SHA: фиксируется commit этой записи; production activation и post-deploy paid smoke этой записью не утверждаются
Результат: publication candidate включает `CLAUDE_API_GEMINI_BATCH_ENABLED=1` и `CLAUDE_API_GEMINI_BATCH_PUBLIC_ENABLED=1` только в blue-green `claude-api-gemini@.service`; legacy singleton rollback unit остается без Batch. Dedicated data keyring не переносится в systemd и читается из root-owned `server.env`. Native ListModels/GetModel рекламируют `batchGenerateContent` только при фактически собранном public facade и только для text-output моделей. Customer docs описывают собственный account-scoped Files API (upload TTL 48ч), Batch-only расширение `fileData`, операционный 85%-потолок 5h-окна и отсутствие отдельной Google Batch скидки/SLA. Caddy/router public perimeter уже покрывает `/v1beta/*` и `/upload/v1beta/*`; regression gate сохраняет оба пути только на Gemini host и unified router.
Отступления от плана: по прямому указанию владельца live/paid Stage 6 run не выполняется в этой задаче. Поэтому §6 post-deploy direct+router create→poll→terminal smoke, soak observation и точный published-SHA GREEN не отмечаются выполненными; эта запись является pre-activation checklist, а не Stage 6 exit verdict.
Измерения: live settlement `$0.000000000`; cumulative Stage 5 settlement остается `15,600 nanoUSD`, остаток подтвержденного Stage 5+6 бюджета остается `9,999,984,400 nanoUSD`. Проверки systemd/discovery/Caddy/router/docs/build/shell фиксируются в commit после локального gate.
Checklist publication: production defaults/systemd — выполнено только для reviewed Gemini slot template; discovery — conditional facade composition + image exclusion; pricing — metering/tariff не менялись, Batch использует existing normal Google tariff/multiplier; apps/web/docs — обновлены; OpenKeys/packages/contracts/apps/admin/commerce pricing/model release — не применимо, новая модель, цена, provider ID, Control API или sale allowlist не добавляются; new alert/metric checklist — не применимо, Stage 5 alerts/metrics не меняются.
Следующий шаг: merge/deploy обычным watchdog pipeline; отдельный операторский шаг после явного разрешения на paid run выполняет direct и router public smoke в оставшемся бюджете, затем фиксирует append-only Stage 6 activation/soak verdict. До этого Stage 6 exit gate остается незакрытым.

## 2026-08-21 — Этап 6 (§6/§9): public activation и Definition of Done GREEN
SHA: published `2a1e942b829fe012c35d306ee1c98283abc57fb0`, `deploy/watchdog` GREEN
Результат: public discovery по `gemini.api.apitoken.sale` и `router.apitoken.sale` вернул 200 и `batchGenerateContent`. Перед каждым smoke worst-case hold `162,000 nanoUSD`, остаток бюджета покрывал оба run. Direct job `batch-83e1c037-…` и router job `batch-3bbd9792-…` выполнены create→poll→`BATCH_STATE_SUCCEEDED`; каждый terminal LRO содержит `response.inlinedResponses[0].response`, item `succeeded`, settlement outbox `done`, actual `5,200 nanoUSD`. Operational soak после drain: queue/active/indeterminate/settlement backlog = 0, authority/public ready. Compatible activation также дочитала ранее принятый `batch-dcd…` (2 items) после restart, доказав rollback/restart compatibility без replay.
Отступления от плана: предыдущая Stage 5 cumulative запись `15,600` не включала позднее restart completion job `batch-dcd…`; эта append-only запись корректирует итог по authoritative DB. Paid partial error не форсировался; deterministic fault matrix остаётся evidence.
Измерения: full history — 6 successful Batch items × `5,200` + ordinary parity `5,200` = cumulative `36,400 nanoUSD = $0.000036400`; account balance `9,999,963,600`, reserved `0`, no stale holds. Остаток aggregate budget `9,999,963,600 nanoUSD = $9.999963600`. Direct/router public smoke 2/2 GREEN; discovery 2/2 GREEN; queue drained.
Следующий шаг: цель Этапов 1–6 и Definition of Done §9 выполнена; поддерживать alerts/retention и не откатываться на binary без Batch authority compatibility.

## 2026-08-20 — Corrective 100k/Ultra delivery
SHA: schema `216e025eb818539f3cdd4b02a4a090315b382a6c` и `88a5513cf6204bdc9ed22d58da5fffee478f3510`, Ultra runtime `07d08686f123fd1e889fb71ac377078265f15a6f`; все `deploy/watchdog` GREEN
Результат: expand-only schema 59 добавила slots 3–20 и durable millisecond pacing authority с promotion в legacy slots для rollback drain. Schema 60 добавила ciphertext-only staging, resumable-file progress, output-build fencing и lifecycle/tombstone поля. Зависимый Ultra runtime использует 20 slots только для `google_ai_ultra|workspace_ai_ultra`, два для остальных/unknown plans, удалил отдельный account cap 16 и ставит durable случайный интервал 2–5 секунд на точной helper socket-send границе после OAuth acquisition. Ordinary Gemini не читает Batch pacing/slots.
Отступления и исправления: первый runtime candidate был RED и не повторялся — real-PG test ожидал terminalized pacing item в queued totals, pacing стоял до OAuth, а early fence error мог не acknowledge helper. Новый SHA исправил причины, прошёл trusted-host real-PG и полный Rust gate. 100k staging/output/upload/expiry consumer доставляется отдельно после обеих GREEN schema.
Следующий шаг: выполнить полный 100k mock/provider-free lifecycle gate и production bounded smoke без отправки 100k paid model turns.
