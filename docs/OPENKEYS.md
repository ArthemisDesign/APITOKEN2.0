...
по метке/ID и пагинацию; открыта может быть одна партия, повторный клик её скрывает. Только для
выбранной партии загружаются и расшифровываются складские секреты (не больше 100), отдельно видны
