...
| `crates/registry` migration 0062 | narrow Grafana rollups `request_fact_usage_top_customer_model_daily`, `request_fact_usage_top_client_daily`, `request_fact_usage_top_model_daily`, and `request_fact_usage_top_tool_daily`; same privacy boundary, grouped one axis at a time to avoid broad monitoring PostgreSQL hash aggregation | no live Grafana consumer yet: `install-monitoring.sh` grants `SELECT` only when `to_regclass` finds the views, and production overview panels 435-438 plus the installer canary still read migration-0061 views because monitoring install runs before engine migrate | `docs/engine/REQUEST_OBSERVABILITY.md`, `docs/ops/MONITORING.md` |
