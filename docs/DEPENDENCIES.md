...
| Producer | Contract / channel | Consumers | Contract document |
|---|---|---|---|
| `crates/server` (`src/http.rs`, `src/admin.rs`) | HTTP `/admin/*` under `x-api-key: CLAUDE_API_CONTROL_KEY`; account creation/readback, status, keys, ledger, scalar default pricing and canonical per-provider overrides. Additive private request analytics exposes bounded summary, keyset drilldown and logical-attempt reads with `scope_version=1`, honest unknown coverage and process-local runtime health; it returns no customer content or raw/opaque money identities. Account and override multipliers are `0..10000`; a charge ledger row keeps full billed `amount_nano` and additively exposes non-negative `uncollected_nano` (missing means zero), so consumers derive the collected debit and never commission pool-funded shortfall. Other additive fields remain ignorable. | `packages/engine-client` is the only TypeScript transport; `apps/api`, `apps/worker` and `apps/openkeys` consume it. Request analytics has no TypeScript consumer until this producer SHA is production GREEN. Direct TypeScript calls to `/admin/*` outside the client remain forbidden. | `docs/engine/CONTROL_API.md`, `docs/commerce/PRICING_MODEL.md`, `docs/product/OPENKEYS.md` |
...
| Producer | Contract / channel | Consumers | Contract document |
|---|---|---|---|
| `apps/api` (`src/sales-feed.controller.ts`, `/v1/internal/sales/*`) | GET feeds `attributions` / `usage-events` / legacy `topups` / additive `topups-v2` / additive `payment-reversals` (cursors `after_id`). Password/OAuth signup commits its referral row with the new account; promo redemption commits attribution before credit. The live usage producer emits only the scalar form: `amountNano=real_funded_nano`, optional informational `providerId`, and null retired attribution/release fields. Usage/topup/reversal eligibility starts at the durable attribution timestamp. Legacy topup pages fail closed at a split equal-`paid_at` group; `topups-v2` uses the fenced commit-ordered `payments.feed_seq`, advances over the full source page before referral filtering, and keeps a verified deposit replayable after a later refund. A terminal payment transaction also appends a fenced immutable reversal audit row; all three money feeds expose a committed `sourceHead`, including rows still inside visibility lag. The authenticated `POST referral-profiles` projection accepts only an explicit bounded user-id list and additively returns each account's authoritative Commerce email without persisting it in Sales. | `apps/sales-api` (`sync.service.ts`) consumes scalar usage through `recordReferredSpend`, storing the informational `providerId` in the separate authority-free `partner_usage_events.spend_provider_id` (migration 0022) so earnings can be reported per provider without touching the retired attribution tuple; its v1/v2 parsers/writers remain only for expand-only replay of historical usage rows. It consumes `topups-v2` for analytics and an independent immutable funding-lot replay, allocates usage/commissions by causal payment FIFO, and consumes `payment-reversals` only after both causal feeds prove their current head. Reversal evidence, exact negative entries and its cursor commit together under `SERIALIZABLE`; crash replay is idempotent and a conflict/incomplete allocation leaves the cursor behind. Gross, signed adjustments, net, debt and payable are separate read fields. Payout prepare/send drain all causal feeds, hold the sync mutex, take the shared accounting lock, re-probe Commerce heads, and keep both fences through commitment/signing; any head/cursor/allocation/reversal mismatch fails closed. The legacy timestamp cursor remains unchanged. The consumer rejects non-canonical/out-of-range bigint values, repeats the attribution-time gate in storage, and globally fences one `commerce_event_id` across recorded and pending v1/v2 stores. `commerce.service.ts` uses `COMMERCE_BASE_URL`; it may consume profile email only after the exact producer SHA is production-GREEN. | `docs/sales/SALES_PORTAL.md` |
| `apps/api` (`src/sales-feed.controller.ts`, `/v1/internal/sales/partner-business-pricing`) | A granted partner or approved partner request prices an owned referral as B2B: commerce independently proves the customer is attributed to the calling partner's referral code and re-checks the discount ceiling. The additive optional `operationRef`/`actorId`/`reason` input and `operationRef`/`idempotentReplay` output supported producer-first rollout. Commerce advisory-locks the ref, rejects canonical payload drift with 409, and commits conversion/default + `customer_provider_discounts` + fenced `engine_pricing_jobs` + component/terminal `audit_log` evidence atomically. Exact retry returns the stored terminal result without another pricing/audit side effect. Retired per-account policy tables are not consulted. | `apps/sales-api`: granted synchronous writes send a stable operation ref when the client supplies `Idempotency-Key`; approved B2B requests always use the durable `partner_request_effects` consumer with a stable effect ref, unique lease fencing, bounded retry, matching-operation acknowledgement and explicit terminal conflicts | `docs/sales/SALES_PORTAL.md`, `docs/sales/PARTNER_PROGRAM.md` |
| `apps/sales-api` (`src/internal.controller.ts`, `/v1/internal/*`) | The legacy `promo/redeem` producer is dormant after removal of the Commerce public redemption consumer; it remains temporarily expand-only so existing redemption/audit history is not rewritten and can be retired only in a later producer-removal stage. `partners/referral-discount` atomically claims a one-time link and returns its promised `discountBps` only to the winning user. `partners/external-referral-alias` idempotently maps one trusted `(source, externalRef)` to an opaque public alias owned by an active ordinary partner; the immutable alias has no price-marker semantics and resolves through the same attribution/commission path. | `apps/api` (`auth.service.ts`, CRM bridge; `SALES_API_URL`); no live promo-redemption consumer | `docs/sales/SALES_PORTAL.md`, `docs/commerce/CRM_BRIDGE.md` |
...
| `apps/openkeys` (`POST /api/usage/lookup`) | Same as before: `POST {key}` → `{viewToken}` plus HttpOnly session cookie on `openkeys.apitoken.sale`, 404 for unknown keys; unchanged and expand-only. | Additive consumer: static page `apps/web/public/landing/b2b.html` (script `b2b.js`) repeats the openkeys key-login flow cross-origin with `credentials: 'include'` and on 200 navigates to `https://openkeys.apitoken.sale/profile`. The route has no CORS/OPTIONS handling, so until openkeys allows the landing origin the browser blocks the call and the page shows the "server unreachable" error — fallback for a successful login remains opening `https://openkeys.apitoken.sale/profile` and entering the key there. | `docs/product/OPENKEYS.md` |

### Other cross-context relationships

| Producer | Contract / channel | Consumers | Contract document |
|---|---|---|---|
| `apps/web` (browser localStorage on `apitoken.sale`) | Shared client-side preference keys between the static landing (`apps/web/public/landing/*`) and the Next.js dashboard (`apps/web/src`). Theme: the landing reads `apitoken-theme` ?? `theme:v1` ?? legacy `theme` and always rewrites `apitoken-theme`; toggles on both sides write `apitoken-theme` + `theme:v1` (the dashboard writes `theme:v1` and reads the same fallback chain — `src/lib/user-preferences.ts`). Language: one canonical key `lang:v1` (`en`|`ru`); landing language links write it before navigation, the RU landing pages redirect to their EN twins while it is unset or `en` (EN is the default), and the dashboard bootstrap (`src/lib/locale-routes.ts`) maps it onto `/ru*` routes. Expand-only: keys and value sets are never renamed or repurposed. | Both sides of `apps/web` on the same origin only; `apps/openkeys` lives on `openkeys.apitoken.sale` with its own keys (`theme`, `openkeys-lang`) and never shares this storage. | This file |
| `packages/contracts` | Zod schemas for engine account/key/ledger/usage, private request-analytics summary/page/logical responses, auth and checkout boundaries plus canonical model pins. Request analytics requires `scope_version=1`, bounded rows/axes, nullable honest evidence, unknown historical denominator/drop attribution and process-local runtime health; additive unknown object fields remain ignorable. The live scalar account contract is `mult_bp=0..10000` for both creation input and account readback; money remains integer/decimal-string safe. | `apps/api`, `apps/worker`, `apps/openkeys`, `packages/db`, `packages/engine-client`. Do NOT import: `apps/web`, `apps/sales-*`, `apps/admin` | `docs/engine/CONTROL_API.md`, `docs/commerce/PRICING_MODEL.md` |
...
| `apps/api` (public API) | HTTPS `backend.apitoken.sale/v1/*`, cookie session. The account-bound `/v1/referral/*` routes derive membership only from that session, resolve Team/B2B targets by current active Commerce email, call Sales server-to-server, and remove Commerce/Sales authority IDs from browser responses. | `apps/web` (`src/lib/api.ts`, `NEXT_PUBLIC_BACKEND_URL`), including Dashboard → Referral only after the exact Sales producer prerequisite below is production-GREEN | `docs/commerce/COMMERCIAL_BACKEND.md`, `docs/sales/PARTNER_PROGRAM.md` |
| `apps/api` (`GET /v1/ready`) | HTTP 200 means this Nest slot can accept traffic: commerce Postgres is up and the process is not draining. JSON still reports `engine` as telemetry; `engine: "down"` is not a load-balancer failure. Control API reachability is not a ready dependency. | Caddy `:8791` `health_uri /v1/ready`; GitHub production-uptime commerce database probe. Engine-backed routes keep their own `503`. | `docs/commerce/COMMERCIAL_BACKEND.md`, `deploy/CADDY.md` |
...
| `apps/api` (admin API) | `/v1/admin/*` via the Caddy rewrite `admin.apitoken.sale/admin/*`, header `x-admin-key`; dedicated no-store request analytics routes `/request-analytics/{summary,logical/:id}` plus `/request-analytics` consume only typed EngineClient reads, validate 30-day/keyset/UUID bounds and preserve the engine's privacy projection without an invented owner join. Additive `GET /events` fans commit-bound `commerce_admin_changes` into resource-prefix SSE invalidations with initial/reconnect resync. Additive `GET /users[].provider_spend_30d` reports exact decimal nanoUSD strings for Claude/GPT/Gemini/Kimi plus an honest residual `other` from immutable commerce usage events; it does not infer image usage from provider identity. `/users/:id/provisioning-repair` reconciles only an existing mapped account after two fresh Control API readbacks: live `active` status and the exact default multiplier must match both commerce copies, then a row-locked CAS admits that exact `pending|error` mapping and writes one replay-idempotent audit event; it never creates/imports/reprices and fails closed for disabled or drifting state. Scalar B2B pricing uses atomic `PATCH /business-users/:id/pricing` plus the fenced durable delivery queue. `GET /admin/pipeline-health` exposes the live pipeline summaries. The same authenticated channel and key are used on `content-studio.apitoken.sale/v1/*`; retired release-cycle/policy/catalog/switch firing routes are not contracts and must not be restored. | engine Control API — via typed `packages/engine-client`; `apps/admin` renders the five compact 30-day provider rails and exact CSV fields only after GREEN producer SHA `51c591ddc9865972bc1514dc08e8c332297b1798`; `apps/content-studio` (`/v1/admin/content/*`) | `docs/commerce/COMMERCIAL_BACKEND.md`, `docs/commerce/PRICING_MODEL.md`, `docs/product/ADMIN_PANEL.md`, `docs/engine/CONTROL_API.md` |
...
| `apps/sales-api` (public + admin API) | Migration 0025 activates individual Team/B2B authority and immutable requests. Admin invite onboarding/patch sets `teamOverrideMaxBps`, `teamInvitesEnabled`, `b2bEnabled`, `b2bMaxDiscountBps`, and `b2bCanDelegate`; application approval additively accepts the same authority and commits partner creation, initial terms, application state and audit evidence atomically. A conflicting existing partner leaves the application pending instead of claiming unapplied terms. An admin B2B change is a direct platform grant. A parent edits only a direct member, never the member's platform direct commission or platform B2B grant. Team override is hard-capped at 2000 bps; inherited B2B is source-bound; narrowing clamps/revokes descendants and pending invites leaf-first. Partner endpoints create/list reasoned commission and owned-referral B2B requests under `Idempotency-Key`; admin endpoints list/detail/decide with `X-Admin-Actor` and mandatory note. Commission approval applies in Sales; B2B approval uses the Commerce effect boundary above. The older `POST /v1/partner/invites` keeps its previous direct-rate semantics only through consumer retirement. `GET /v1/partner/earnings/providers` returns daily UTC provider points. Referral, Team, request and managed-admin views use Commerce-authoritative/account email with a short-mask outage fallback; additive `GET /v1/admin/payout-list.items[].email` makes the same email-first identity available to payout previews without changing immutable `partnerId` payout authority. Active web consumers expose no promo-code controls; the old backend promo boundary remains expand-only compatibility until producer-first retirement. `partners.apitoken.sale/v1/*`; `/v1/admin` via Caddy `admin.apitoken.sale/partner-admin/*`, header `x-sales-admin-key`; additive `GET /events` fans commit-bound `sales_admin_changes` into resource-prefix SSE invalidations with initial/reconnect resync | `apps/sales-web`; `apps/admin` | `docs/sales/SALES_PORTAL.md`, `docs/sales/PARTNER_PROGRAM.md`, `docs/product/ADMIN_PANEL.md` |
| `apps/sales-api` (`src/commerce-partner.controller.ts`, `/v1/internal/referral/*`) | Commerce-account partner producer under `SALES_CONTROL_KEY`. Membership resolve activates only an unexpired UUID-bound invite; immediate admin onboarding supersedes a pending invite; Team creation/edit is serialized and bounded by the inviter's platform authority. Partner snapshots expose ledger/request/payout/provider data keyed by immutable Commerce UUID but no promo/Telegram product authority. Internal request views add nullable `customerCommerceUserId` so the server-side consumer can recover authoritative customer email after a reload; legacy browser contracts do not receive it. Direct B2B pricing reuses the fenced Sales→Commerce operation-ref boundary. New usage commission rows use `calculation_version=2`: each parent cut is withheld from the fixed direct gross pool, `net=gross-withheld`, and database guards require active memberships whose start precedes the event; total platform payout is conserved. Legacy rows remain version 1 and `program_enabled=false`. | No Commerce consumer until this exact producer SHA is production-GREEN. Then `apps/api` owns `/v1/referral/*`, derives `commerceUserId` only from its verified session/admin user lookup, enriches UUIDs with current email and strips internal IDs before browser responses. `apps/web` and `apps/admin` consume only Commerce routes. | `docs/sales/SALES_PORTAL.md`, `docs/sales/PARTNER_PROGRAM.md` |
...
| Chatwoot (`support.apitoken.sale`, host process on `127.0.0.1:3010`) | webhook `POST /hooks/devbot/{DEVBOT_CHATWOOT_SECRET}` — event `message_created`; Caddy on `support.apitoken.sale` reverse-proxies that prefix to `127.0.0.1:3800` with `log_skip`; incoming contact messages only (outgoing/private/activity dropped); optional HMAC `X-Chatwoot-Signature` when `DEVBOT_CHATWOOT_HMAC_SECRET` is set | `apps/devbot` (💬 Support topic) | `docs/ops/DEVBOT.md` |
| `apps/api` (commerce) | fail-open loopback webhook `POST http://127.0.0.1:3800/hooks/partners/{DEVBOT_PARTNER_SECRET}` when `DEVBOT_PARTNER_WEBHOOK_URL` is set; events `submitted` / `updated` / `decided`; no `userId`; HTTP loopback only | `apps/devbot` (🤝 Partners topic) | `docs/ops/DEVBOT.md` |
...
| `apps/devbot` | loopback `/metrics` (`127.0.0.1:3800/metrics`, Prometheus job `devbot`): `devbot_telegram_send_failures_total`, `devbot_last_webhook_seconds`, `devbot_last_chatwoot_seconds`, `devbot_last_partner_seconds` | Prometheus → alerts `DevBotTelegramSendFailures`, `DevBotWebhookSilent`, `DevBotMetricsDown`. `devbot_last_chatwoot_seconds` and `devbot_last_partner_seconds` are operational only (silence is normal when no clients write / no applications arrive) | `docs/ops/MONITORING.md`, `docs/ops/DEVBOT.md` |
| `apitoken-affinity-redis.service` | two instances under one unit: history `127.0.0.1:6379` (legacy Compose service identity `affinity-redis`, Codex response history, `allkeys-lru`, 8 GiB) and affinity `127.0.0.1:6380` (service `cache-affinity-redis`, cache-lineage L2 + advisory cooling hints `claude-api:cool:v1`, `allkeys-lru`, 128 MiB); the installer performs an additive Compose reconcile up to the two-target monitoring gate and does not stop history; exporters `9121`/`9122` (job `redis`, label `instance_role`); the password is read from `CLAUDE_API_REDIS_PASSWORD` in `server.env` and published as the JSON secret `observability/secrets/affinity_redis_password` | Prometheus → alerts `AffinityRedisDown`, `AffinityRedisEvictingKeys`, `AffinityRedisMemoryHigh` | `docs/ops/MONITORING.md#affinityredisevictingkeys` |
...
| `crates/registry` migration 0061 | PostgreSQL views `request_fact_usage_daily` and `request_fact_tool_usage_daily`: private daily aggregates that join request facts to authoritative usage by billing request ID and expose opaque engine account/non-secret key identity, normalized client/model/closed-tool dimensions and token/nanoUSD totals; no email, key label, raw key, request, content or arbitrary tool identity | Grafana datasource `engine-request-analytics` through role `apitoken_monitoring`, dashboard uid `apitoken-request-usage`; no other consumer and no base-table grant | `docs/engine/REQUEST_OBSERVABILITY.md`, `docs/ops/MONITORING.md` |
| `crates/registry` migration 0062 | narrow Grafana rollups `request_fact_usage_top_customer_model_daily`, `request_fact_usage_top_client_daily`, `request_fact_usage_top_model_daily`, and `request_fact_usage_top_tool_daily`; same privacy boundary, grouped one axis at a time to avoid broad monitoring PostgreSQL hash aggregation | Grafana datasource `engine-request-analytics` through role `apitoken_monitoring`, production overview panels 435-438; `install-monitoring.sh` grants `SELECT` only when `to_regclass` finds the views; installer canary prefers `request_fact_usage_top_model_daily` and falls back to the 0061 daily view on a first-boot host; no base-table grant | `docs/engine/REQUEST_OBSERVABILITY.md`, `docs/ops/MONITORING.md` |
...
- **Policy preflight contract (provider planes → router, phase 6.4a).** The producer is
  ...
  8790/8792/8794/8803 are reachable by the router over loopback. Router-owned preset expansion and
...
| (loopback only, no public vhost) | KIMI runtime `:8803` (active/passive slots 8804/8805); backend-only plane. Customer `kimi/*` is routed here by `claude-router` (`CLAUDE_ROUTER_KIMI_ORIGIN`); there is no `kimi.*` hostname |
...
| `monitoring.apitoken.sale` | managed session with `/__admin-auth/*` on commerce auth 8791; Grafana `:3600` |
| `support.apitoken.sale` | Chatwoot `:3010`; `/hooks/devbot/*` → `apps/devbot` `:3800` (incoming client-message notifications; Caddy `log_skip`) |
