...
| `crates/server` Gemini status producer | additive protected `/gemini-subs.credential_generation_digest`, an opaque path-independent identity of the exact encrypted credential set loaded by the process; equality is the only supported interpretation | producer-first contract for the private root Gemini admission controller; ordinary panel/Auth Bot consumers ignore the additive field | `docs/engine/CONTROL_API.md`, `docs/engine/GEMINI_PROVIDER.md` |
