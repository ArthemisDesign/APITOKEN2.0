...
| `apps/api` (paid-customer admin producer) | Read-only `GET /v1/admin/finance/paying-users` через Caddy-rewrite `admin.apitoken.sale/admin/*`: только клиенты с подтверждённым платежом, серверная пагинация/фильтры и exact nanoUSD provider-spend за 1/7/30 дней | `apps/admin` (`/paying-users`), подключён отдельным consumer-коммитом после GREEN producer SHA `ce92503d1adc0e31967b2dda5853ce05ed480048` | `docs/commerce/COMMERCIAL_BACKEND.md`, `docs/product/ADMIN_PANEL.md` |
