...
| `crates/authbot` (`tripo3d_roster.rs`, `AUTH_BOT_TRIPO3D_{DIR,CREDENTIAL_KEYS,CREDENTIAL_ACTIVE_KID}`) | Sealed Tripo3D roster at `/srv/claude-api/data/tripo3d` (`profiles.json` + `credentials/<id>.json`, XChaCha20-Poly1305 envelopes AAD-bound to profile id): atomic publish/replace after the free balance probe and cohort corroboration; the engine only ever reads | `crates/forward` (`tripo3d/roster.rs`) on the dedicated default-off `ProviderMode::Tripo3d` plane (`CLAUDE_API_TRIPO3D_*`); `crates/server` read-only `GET /tripo3d-subs` (control key) projects it to the admin surface — privacy-safe, no key/subject/proxy | `docs/engine/TRIPO3D_PROVIDER.md` |
| `crates/authbot` (`suno_roster.rs`, `AUTH_BOT_SUNO_{DIR,CREDENTIAL_KEYS,CREDENTIAL_ACTIVE_KID}`) | Sealed Suno roster at `/srv/claude-api/data/suno` (`profiles.json` + `credentials/<id>.json`, XChaCha20-Poly1305 envelopes AAD-bound to profile id): atomic publish/replace after Clerk session discovery → JWT mint → free billing probe → plan corroboration; the engine reads and additionally RE-SEALS a profile's envelope when a `set-cookie` rotation arrives (single-flight, manifest §2) | `crates/forward` (`suno/roster.rs`, `suno/session.rs`) on the dedicated default-off `ProviderMode::Suno` plane (`CLAUDE_API_SUNO_*`); `crates/server` read-only `GET /suno-subs` (control key) projects it to the admin surface — privacy-safe, no cookie/session/subject/proxy | `docs/engine/SUNO_PROVIDER.md` |
