...
|---|---|---|---|
| `crates/server` (`src/http.rs`, `src/admin.rs`) | HTTP `/admin/*` under `x-api-key: CLAUDE_API_CONTROL_KEY`; routes only in Combined/Anthropic modes; release-v2 policy reads include exact-version and newest complete immutable version for one policy ID | `packages/engine-client` — the only client; the latest-policy consumer is wired only after this producer's GREEN exact SHA and direct calls to `/admin/*` outside it are forbidden | `docs/engine/CONTROL_API.md` |
