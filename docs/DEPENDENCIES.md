...
| `crates/server` Gemini status producer + `crates/forward` native Gemini producer | additive protected `/gemini-subs.credential_generation_digest`, an opaque path-independent identity of the exact encrypted credential set loaded by the process; equality is the only supported interpretation. The dormant `gemini-3.7-flash` native lane additively accepts the admin-only exact profile/request-id/deadline tuple (`x-apitoken-calibration-not-after`, canonical positive Unix seconds), preserves raw JSON/SSE upstream `modelVersion`, and returns only a strictly pre-deadline canonical epoch-ms `x-apitoken-calibration-dispatch-ms` attested at the SHA-pinned Node pre-`_flush` socket boundary; count may refresh once, paid generation is cached-bearer-only, and every exact POST is non-replayable | producer-first contract for the private root Gemini admission controller. Its status-digest and future deadline/identity consumer/trigger remain deferred until the exact producer SHA is watchdog-GREEN; this change does not wire that consumer, publish the model or enable a live call. Ordinary panel/Auth Bot consumers ignore the additive status field, and ordinary Gemini responses never carry the private dispatch header | `docs/engine/CONTROL_API.md`, `docs/engine/GEMINI_PROVIDER.md` |
