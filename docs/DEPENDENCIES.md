...
| `apps/api` (`src/sales-feed.controller.ts`, `/v1/internal/sales/partner-business-pricing`) | A granted partner prices their own referral as B2B: commerce independently proves the customer is attributed to the calling partner's referral code and re-checks the discount ceiling. The additive optional `operationRef`/`actorId`/`reason` input and `operationRef`/`idempotentReplay` output support producer-first rollout; the dependent durable Sales consumer must always send its effect key and authenticated actor after this producer SHA is production-GREEN. Commerce advisory-locks the ref, rejects canonical payload drift with 409, and commits conversion/default + `customer_provider_discounts` + fenced `engine_pricing_jobs` + component/terminal `audit_log` evidence atomically. Exact retry returns the stored terminal result without another pricing/audit side effect. Retired per-account policy tables are not consulted. | `apps/sales-api` (`partner.controller.ts` → `commerce.service.ts`), gated on `partners.b2b_enabled`/`b2b_max_discount_bps`; the request-effect outbox consumer remains dormant until the exact producer SHA is GREEN | `docs/sales/SALES_PORTAL.md`, `docs/sales/PARTNER_PROGRAM.md` |
...
| `apps/sales-api` (public + admin API) | B2B grant: `PATCH /v1/admin/partners/:id` and `POST /v1/admin/invites` accept `b2bEnabled`/`b2bMaxDiscountBps`, storing an admin-set ceiling on the partner row (migration 0023, off by default); the grant is authority for a partner to price their OWN referrals as B2B and does not alter commission, which stays a percentage of the customer's own money for both classes. Partner Team is additive after production migration 0024: authenticated `GET /v1/partner/team` and `GET /v1/partner/invites` expose the fixed platform direct rate, exact child edge, delegated ceiling and accounting; new `POST /v1/partner/team/invites` creates a one-time 30-day Telegram-bound invite whose direct rate is the configured 10% default while `overrideBps`/`teamOverrideMaxBps` are bounded by the inviter and the 20% hard ceiling. The older `POST /v1/partner/invites` keeps its previous direct-rate semantics only through producer/consumer rollout and is disabled in a later retirement release. `PATCH /v1/partner/team/:memberId` changes only a direct edge/delegation and atomically clamps a lowered subtree. `GET /v1/partner/earnings/providers` additively returns daily UTC provider points, retaining unknown provider history. Referral lists, managed-admin detail and activity additively expose Commerce-authoritative email with short-mask outage fallback. Admin create/patch accepts `teamOverrideMaxBps`. Active web consumers expose no promo-code issuance/redemption controls; the old backend promo boundary remains historical expand-only compatibility until producer-first retirement. `partners.apitoken.sale/v1/*`; `/v1/admin` via Caddy `admin.apitoken.sale/partner-admin/*`, header `x-sales-admin-key`; additive `GET /events` fans commit-bound `sales_admin_changes` into resource-prefix SSE invalidations with initial/reconnect resync | `apps/sales-web`; `apps/admin` | `docs/sales/SALES_PORTAL.md`, `docs/sales/PARTNER_PROGRAM.md`, `docs/product/ADMIN_PANEL.md` |
