...
| `apps/api` (paid-customer admin producer) | Read-only `GET /v1/admin/finance/paying-users` через Caddy-rewrite `admin.apitoken.sale/admin/*`: только клиенты с подтверждённым платежом, серверная пагинация/фильтры и exact nanoUSD provider-spend за 1/7/30 дней | после GREEN producer SHA: `apps/admin` отдельным consumer-коммитом; до него прямых потребителей нет | `docs/commerce/COMMERCIAL_BACKEND.md`, `docs/product/ADMIN_PANEL.md` |
