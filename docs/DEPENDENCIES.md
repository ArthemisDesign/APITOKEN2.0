...
| `apps/openkeys` (pricing inventory producer) | loopback/internal GET `/api/internal/pricing/v2/inventory`, bounded cursor + full `sha256:v2` manifest под `X-OpenKeys-Control-Key`; без secrets/live money | `packages/db` Stage 5 v2 consumer исчерпывает cursor дважды и требует один неизменный full-manifest digest; подключён только после GREEN producer SHA | `docs/product/OPENKEYS.md`, `docs/commerce/MULTI_DISCOUNT_STAGE5.md` |
