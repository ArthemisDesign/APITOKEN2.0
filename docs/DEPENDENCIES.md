...
| `apps/devbot` | loopback `/metrics` (`127.0.0.1:3800/metrics`, Prometheus job `devbot`): `devbot_telegram_send_failures_total`, `devbot_last_webhook_seconds`, `devbot_last_chatwoot_seconds` | Prometheus → alerts `DevBotTelegramSendFailures`, `DevBotWebhookSilent`, `DevBotMetricsDown`. `devbot_last_chatwoot_seconds` is operational only (silence is normal when no clients write) | `docs/ops/MONITORING.md`, `docs/ops/DEVBOT.md` |
