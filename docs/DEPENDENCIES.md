...
| `crates/server` Gemini status producer + `crates/forward` native Gemini producer | additive protected `/gemini-subs.credential_generation_digest`, an opaque path-independent identity of the exact encrypted credential set loaded by the process; equality is the only supported interpretation. The dormant `gemini-3.7-flash` lane requires the admin-only exact profile/UUIDv4/deadline tuple, returns only a strictly pre-deadline canonical producer `dispatch-ms` attested at the SHA-pinned Node pre-`_flush()` boundary, preserves raw upstream `modelVersion`, permits one non-replayable count refresh and requires cached bearer/no replay for paid generation | the generic `tools/gemini_calibration/run_live.py` runner is the only retained live-runner base. A future reviewed extension may consume the dormant contract through an operator-launched exact-SHA non-public canary; the retired root bridge, trigger and admission unit are not consumers. No live call, public catalogue/default/router contract or storefront exposure is enabled. Ordinary panel/Auth Bot consumers ignore the additive status field, and ordinary Gemini responses never carry the private headers | `docs/engine/CONTROL_API.md`, `docs/engine/GEMINI_PROVIDER.md`, `docs/ops/GEMINI_CALIBRATION.md`, `research/GEMINI_3_7_FLASH.md` |
