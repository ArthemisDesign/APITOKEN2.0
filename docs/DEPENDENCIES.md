...
| `apitoken-affinity-redis.service` | два инстанса под одним юнитом: history `127.0.0.1:6379` (Codex response history, `volatile-lru`, 384 MiB) и affinity `127.0.0.1:6380` (cache-lineage L2, `allkeys-lru`, 128 MiB); экспортеры `9121`/`9122` (job `redis`, лейбл `instance_role`); пароль читается из `CLAUDE_API_REDIS_PASSWORD` в `server.env` и публикуется как JSON-секрет `observability/secrets/affinity_redis_password` | Prometheus → алерты `AffinityRedisDown`, `AffinityRedisEvictingKeys`, `AffinityRedisMemoryHigh` | `docs/ops/MONITORING.md#affinityredisevictingkeys` |
