...
| `crates/registry` migration 0062 | narrow Grafana rollups `request_fact_usage_top_customer_model_daily`, `request_fact_usage_top_client_daily`, `request_fact_usage_top_model_daily`, and `request_fact_usage_top_tool_daily`; same privacy boundary, grouped one axis at a time to avoid broad monitoring PostgreSQL hash aggregation | Grafana datasource `engine-request-analytics`, production overview dashboard panels 435-438; no other consumer and no base-table grant | `docs/engine/REQUEST_OBSERVABILITY.md`, `docs/ops/MONITORING.md` |
