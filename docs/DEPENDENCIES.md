...
| Producer | Contract / channel | Consumers | Contract document |
|---|---|---|---|
| `apps/api` (`src/sales-feed.controller.ts`, `/v1/internal/sales/*`) | GET feeds `attributions` / `usage-events` / legacy `topups` / additive `topups-v2` / additive `payment-reversals` (cursors `after_id`). Password/OAuth signup commits its referral row with the new account; promo redemption commits attribution before credit. The live usage producer emits only the scalar form: `amountNano=real_funded_nano`, optional informational `providerId`, and null retired attribution/release fields. Usage/topup/reversal eligibility starts at the durable attribution timestamp. Legacy topup pages fail closed at a split equal-`paid_at` group; `topups-v2` uses the fenced commit-ordered `payments.feed_seq`, advances over the full source page before referral filtering, and keeps a verified deposit replayable after a later refund. A terminal payment transaction also appends a fenced immutable reversal audit row; `payment-reversals` advances over its source page before filtering and exports exact original payment nanoUSD. The three money pages add `sourceHead`: the latest committed source id including rows inside the visibility lag; consumers never advance a cursor to it. | `apps/sales-api` (`sync.service.ts`) consumes scalar usage through `recordReferredSpend`; its v1/v2 parsers/writers remain only for expand-only replay of historical usage rows. It consumes `topups-v2` for both analytics and an independent zero-origin immutable funding-lot replay, allocates usage/commissions by causal payment FIFO, and consumes `payment-reversals` only after both causal source feeds prove their current head. Reversal evidence, exact negative entries and its cursor commit together under `SERIALIZABLE`; crash replay is idempotent and a conflict or incomplete allocation leaves the cursor behind. The current consumer ignores additive `sourceHead` until its separate rollout. Signed entries are intentionally not yet included in payout/read math. The legacy timestamp cursor remains unchanged. The consumer rejects non-canonical/out-of-range bigint values, repeats the attribution-time gate in storage, and globally fences one `commerce_event_id` across recorded and pending v1/v2 stores. `commerce.service.ts` uses `COMMERCE_BASE_URL`. | `docs/sales/SALES_PORTAL.md` |
