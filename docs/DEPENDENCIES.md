...
| GREEN Gemini producer `264363f7838ddd2d156b14668a320047ad33b6ee`: `crates/server` status + `crates/forward` native lane | additive protected `/gemini-subs.credential_generation_digest`, an opaque path-independent identity of the exact encrypted credential set loaded by the process; equality is the only supported interpretation. The dormant `gemini-3.7-flash` lane requires the admin-only exact profile/UUIDv4/deadline tuple, returns only a strictly pre-deadline canonical producer `dispatch-ms` attested at the SHA-pinned Node pre-`_flush()` boundary, preserves raw upstream `modelVersion`, permits one non-replayable count refresh and requires cached bearer/no replay for paid generation. The bridge pins the sealed producer, private canary, exact plan, event/usage contract and every controller artifact; the canary fixes hot tariff overrides off so preflight and settlement use the compiled official schedule. Its separate exact one-file trigger is accepted only from the full uninstalled protected-`master` range | fixed root `deploy/watchdog-infrastructure.sh` directly invokes the private admission gate before its durable baseline. The gate may make one free `countTokens` and one paid SSE generation, then every re-entry is offline-only. The consumer is wired but the trigger remains absent; no live call, public catalogue/default/router contract or storefront exposure is enabled. Ordinary panel/Auth Bot consumers ignore the additive status field, and ordinary Gemini responses never carry the private headers | `docs/engine/CONTROL_API.md`, `docs/engine/GEMINI_PROVIDER.md`, `docs/ops/GEMINI_CALIBRATION.md`, `docs/ops/DEPLOYMENT.md`, `research/GEMINI_3_7_FLASH.md` |
