...
| `crates/server` Gemini status producer + `crates/forward` native Gemini producer | additive protected `/gemini-subs.credential_generation_digest`, an opaque path-independent identity of the exact encrypted credential set loaded by the process; equality is the only supported interpretation. The dormant public `gemini-3.7-flash` lane maps only to the credential-bound catalogue row `gemini-3.7-flash-tiered`, requires the admin-only exact profile/UUIDv4/deadline tuple, returns only a strictly pre-deadline canonical producer `dispatch-ms` attested at the SHA-pinned Node pre-`_flush()` boundary, preserves raw upstream `modelVersion`, permits one non-replayable count refresh and requires cached bearer/no replay for paid generation | the generic `tools/gemini_calibration/run_live.py` runner is the only retained live-runner base. Its closed `--gemini-37-admission` mode consumes the dormant contract through an operator-launched exact-SHA non-public canary with one non-retried count and at most one non-replayed paid SSE generation; the retired root bridge, trigger and admission unit are not consumers. The 2026-08-15 report withdrew runtime `20d945ce…` after one count response with valid dispatch attestation and one upstream 404 generation response, so the runner rejects that exact SHA before network I/O and no public catalogue/default/router contract or storefront exposure is enabled. A later free owned catalogue snapshot exposed the new positive tiered row on Pro and Ultra, authorizing a materially different hidden candidate but not publication. A future live requires a different runtime SHA and fresh explicit numeric budget. Ordinary panel/Auth Bot consumers ignore the additive status field, and ordinary Gemini responses never carry the private headers | `docs/engine/CONTROL_API.md`, `docs/engine/GEMINI_PROVIDER.md`, `docs/ops/GEMINI_CALIBRATION.md`, `research/GEMINI_3_7_FLASH.md` |
