...
| `apps/api` (`src/sales-feed.controller.ts`, `/v1/internal/sales/partner-business-pricing`) | A granted partner prices their own referral as B2B: commerce independently proves the customer is attributed to the calling partner's referral code and re-checks the discount ceiling before writing, then reuses `setBusinessPricingBundle` (account default + `customer_provider_discounts`, durable `engine_pricing_jobs`). Retired per-account policy tables are not consulted. | `apps/sales-api` (`partner.controller.ts` → `commerce.service.ts`), gated on `partners.b2b_enabled`/`b2b_max_discount_bps` | `docs/sales/SALES_PORTAL.md`, `docs/sales/PARTNER_PROGRAM.md` |
