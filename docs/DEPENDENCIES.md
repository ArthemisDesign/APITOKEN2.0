...
| `apps/api` (paid-customer admin producer) | Read-only `GET /v1/admin/finance/paying-users` via the Caddy rewrite `admin.apitoken.sale/admin/*`: only customers with a confirmed payment, server-side pagination/filters, and exact nanoUSD provider spend over 1/7/30 days; provider authority is immutable pricing attribution or the stored top-level engine ledger provider, no model-name inference | `apps/admin` (`/paying-users`), wired in a separate consumer commit after GREEN producer SHA `ce92503d1adc0e31967b2dda5853ce05ed480048` | `docs/commerce/COMMERCIAL_BACKEND.md`, `docs/product/ADMIN_PANEL.md` |
