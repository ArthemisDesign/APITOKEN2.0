...
| `crates/server` operator routes | read-only `/overview /capacity /metrics /subs /spend-stats /fleet-history /settlement-health /glm-subs` (→ 8790; GLM is a backend inside the Anthropic runtime), `/codex-subs` (→ 8792), `/gemini-subs` (→ 8794), `/kimi-subs` (→ 8803, a dedicated default-off KIMI plane; the Anthropic-embedded gateway is dev/test only) via Caddy `admin.apitoken.sale`, the proxy injects the key (`ADMIN_CONTROL_KEY`); Claude `/capacity`, `/overview` supply, and Prometheus use one exact turn+quota authority, not pool prior/EMA; the last-known per-window display-state routable-idle/quota-cooling of a subscription is kept only until the exact provider reset and does not make stale remaining saleable | `apps/admin` (no engine-client and no secrets of its own); `/metrics` is also scraped by Prometheus directly over loopback, bypassing Caddy (`observability/prometheus/prometheus.yml`), including the KIMI origin 8803 with the target label `provider: kimi` | `docs/product/ADMIN_PANEL.md`, `docs/engine/CONTROL_API.md` |
...
| `apps/api` (paying/bonus-spender admin producer) | Read-only `GET /v1/admin/finance/paying-users` via the Caddy rewrite `admin.apitoken.sale/admin/*`: omitted funding preserves the payment/manual union; additive `payments|manual|bonus|all` selects a lifetime money cohort and/or strict selected-window bonus-only users proven solely by complete `policy_v1|release_v2` funding splits; exact nanoUSD funding/provider spend over 1/7/30 days, server-side pagination/filters; provider authority is attribution provider falling back to stored top-level engine ledger provider, never model inference | `apps/admin` (`/paying-users`) keeps the prior contract and may consume the additive bonus cohort only in a separate consumer commit after GREEN exact producer SHA (the original consumer was wired after GREEN `ce92503d1adc0e31967b2dda5853ce05ed480048`) | `docs/commerce/COMMERCIAL_BACKEND.md`, `docs/product/ADMIN_PANEL.md` |
...
| `apps/openkeys` (admin API) | `/api/internal/admin/*` via Caddy `admin.apitoken.sale/openkeys-admin/*`, headers `X-OpenKeys-Control-Key` + verified actor; additive read-only `GET /paying-keys` returns only delivered, non-removed keys with DB count/search/pagination before bounded live usage for `1|7|30` days, exact nanoUSD/model counters, and row-local unavailable status without secrets | `apps/admin`, wired to `/paying-keys` only in a separate consumer checkpoint after the exact producer SHA has a GREEN `deploy/watchdog` | `docs/product/OPENKEYS.md` |
