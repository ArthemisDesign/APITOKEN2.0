...
|---|---|---|---|
| `crates/server` (`src/http.rs`, `src/admin.rs`) | HTTP `/admin/*` under `x-api-key: CLAUDE_API_CONTROL_KEY`; routes only in Combined/Anthropic modes; release-v2 policy reads include exact-version and newest complete immutable version for one policy ID | after producer SHA `a7fbd16a0d63b3b16f7049f8aa1ac5b6e739583c` received exact `deploy/watchdog` GREEN, `packages/engine-client` became the only typed latest-policy client and `packages/db` Stage 5 became its double-read fail-closed consumer; direct calls to `/admin/*` outside the client remain forbidden | `docs/engine/CONTROL_API.md`, `docs/commerce/MULTI_DISCOUNT_STAGE5.md` |
...
| `crates/server` + `crates/forward` Stage 8 capture producer | protected `POST /admin/pricing/v2/stage8-evidence/capture`; strict explicit inputs, server-owned compile-fixed manifest, unwrapped schema-v2 report including `passed=false`; PostgreSQL bounded reader only | the durable commerce consumer (strict `packages/contracts` → raw-text/`json-bigint` `packages/engine-client` → `apps/worker` capture lane) was removed with the dismantled release cycle; the remaining caller is the manual release-advance path in `docs/ops/MODEL_RELEASE_CYCLE.md` | `docs/engine/CONTROL_API.md`, `docs/ops/DEPLOYMENT.md`, `docs/commerce/MULTI-DISCOUNT_STAGE9.md` |
...
Stage 6 application consumer (`packages/db/src/funding-normalization-jobs.ts`) were deleted with
the dismantled release cycle (`docs/ops/MODEL_RELEASE_CYCLE.md`). The `packages/db` materializer
and funding libraries and their package CLIs remain in the tree pending a follow-up cleanup
commit; they no longer have an HTTP producer or a worker consumer and must not be wired to new
callers.
...
engine producers — the read-only `POST /admin/pricing/v2/stage8-evidence/capture` and the
evidence-gated single-head `POST /admin/pricing/v2/activate` CAS — remain the release authority
and are driven manually per the runbook; the `packages/db` capture/activation/orchestration
stores remain pending the follow-up cleanup commit. The engine-side invariants (fresh combined
evidence, double-scan authority, exact-replay ACK recovery, receipt-derived recovery/successor
expectations) are unchanged and documented in `docs/engine/CONTROL_API.md`.
...
were deleted with it. The engine `locked-openkeys-transition` endpoint and the
`packages/db/src/pricing-shadow-rollout-jobs-v2.ts` store remain as contract surface pending the
follow-up cleanup, with no live commerce consumer.
...
| `apps/api` (admin API) | `/v1/admin/*` via the Caddy rewrite `admin.apitoken.sale/admin/*`, header `x-admin-key`; exact pre-cutover `/pricing-policy-delivery-repairs` supersedes only a proven dead `strict + legacy_single` job and creates a new audited shadow delivery; `/pricing-catalog-jobs/stage` and `/pricing-switch-jobs/stage` stage delivery for exactly the stored immutable version (no wire payload, exact replay is idempotent) and move only commerce heads; per-service CAS `/service-account-inventory/*` cross-checks the full engine inventory and after cutover completes the exact `meter_only` release-v2 policy/extension before durable registration; `/users/:id/provisioning-repair` re-runs provisioning materialization for a stuck registration (pending/error engine account; the source policy re-pins itself to the live catalog head and a dead delivery is re-materialized); `/users/:id/policy-enforcement-cutover` orchestrates the per-account shadow→strict cutover for one converted B2B client (shared `packages/engine-client` preflight: engine funding normalization plan/apply, exact active-policy ACK stamps on every active key; durable strict job via `stageAccountStrictCutoverJob`; the `apps/worker` delivery re-stamps keys on the new head), and the same chain also runs without the endpoint — B2C→B2B conversion and every `b2b_client` policy save arm `account_policy_bindings.strict_chain_pending` in `packages/db`, and the `apps/worker` pricing sweep advances the same preflight + staging via `packages/db/src/strict-chain.ts` once the exact saved version confirms under shadow; the same channel and key on `content-studio.apitoken.sale/v1/*`. The release-cycle routes (`/pricing-stage5-v2*`, `/pricing-stage6-v2*`, `/pricing-stage8-capture-v2*`, `/pricing-shadow-rollout-v2*`, `/pricing-release-activation-v2*`, `/pricing-release-orchestration-v2*`) were removed with the dismantled release cycle — prices are hot tariff overrides and discounts are hot policy rules, so the cycle's only remaining effect was a hazardous head CAS | `packages/db` materializer libraries (pending the follow-up cleanup commit); engine Control API — via typed `packages/engine-client`; `apps/content-studio` (`/v1/admin/content/*`) | `docs/ops/MODEL_RELEASE_CYCLE.md`, `docs/ops/DEPLOYMENT.md`, `docs/commerce/PRICING.md`, `docs/product/ADMIN_PANEL.md`, `docs/engine/CONTROL_API.md` |
