...
| `crates/server` Gemini status producer + `crates/forward` native Gemini producer | additive protected `/gemini-subs.credential_generation_digest`, an opaque path-independent identity of the exact encrypted credential set loaded by the process; equality is the only supported interpretation. The dormant public `gemini-3.7-flash` lane maps only to the credential-bound catalogue row `gemini-3.7-flash-tiered`, requires the admin-only exact profile/UUIDv4/deadline tuple, returns only a strictly pre-deadline canonical producer `dispatch-ms` attested at the SHA-pinned Node pre-`_flush()` boundary, preserves raw upstream `modelVersion`, permits one non-replayable count refresh and requires cached bearer/no replay for paid generation. After the withdrawn 256-token tiered attempt, a successor exact producer accepts only explicit `maxOutputTokens=512`; the old bound or any other explicit value is rejected before dispatch | the generic `tools/gemini_calibration/run_live.py` runner is the only retained live-runner base. Its closed `--gemini-37-admission` mode consumes the dormant contract through an operator-launched exact-SHA non-public canary with one non-retried count and at most one non-replayed paid SSE generation; the retired root bridge, trigger and admission unit are not consumers. The 2026-08-15 reports withdrew runtime `20d945ce…` after upstream 404 and tiered runtime `2c8aca0d…` after reconciled `MAX_TOKENS`; the runner rejects both exact SHAs before network I/O. Production-GREEN successor `c4f0773a…` then completed one positive count and one no-retry paid SSE generation with exact output, terminal usage, 8/7 incremental frames, confirmed raw `gemini-3.7-flash-tiered` identity and reconciled `$0.0018075`. The person's approved identity contract treats only public `gemini-3.7-flash` or that confirmed alias as admission evidence; ordinary responses rewrite the alias to the public id. Its `$0.788352` authorization is consumed and no SHA may replay it. Public catalogue/default/router and storefront exposure remains a separate publication contract. Ordinary panel/Auth Bot consumers ignore the additive status field, and ordinary Gemini responses never carry the private headers | `docs/engine/CONTROL_API.md`, `docs/engine/GEMINI_PROVIDER.md`, `docs/ops/GEMINI_CALIBRATION.md`, `research/GEMINI_3_7_FLASH.md` |
