...
| `crates/server` + `crates/forward` Stage 8 capture producer | protected `POST /admin/pricing/v2/stage8-evidence/capture`; strict explicit inputs, server-owned compile-fixed manifest, unwrapped schema-v2 report including `passed=false`; PostgreSQL bounded reader only | no live caller remains: the durable commerce consumer was removed with the dismantled release cycle, and the manual release-advance path was retired with head 55 as the final pricing release (`docs/ops/MODEL_RELEASE_CYCLE.md`); the endpoint stays as expand-only contract surface | `docs/engine/CONTROL_API.md`, `docs/ops/DEPLOYMENT.md`, `docs/commerce/MULTI-DISCOUNT_STAGE9.md` |
...
but are no longer driven: the manual release-advance runbook was retired with head 55 as the
final pricing release, so they stay only as expand-only contract surface; the `packages/db`
capture/activation/orchestration stores are deleted. The engine-side invariants (fresh combined
