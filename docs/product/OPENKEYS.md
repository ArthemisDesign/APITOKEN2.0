...
Истёкшая сессия не перерисовывает форму входа молча: на ней показывается явное сообщение
«Сессия истекла — войдите снова». Падение конфигурации env (например, потерянный
`OPENKEYS_SESSION_SECRET`) отвечает тем же 401, но обязательно пишется в серверный лог
(`openkeys admin session check failed`), чтобы мисконфиг не маскировался под истёкшую сессию.
...
Если выпуск заблокирован (`GET /api/admin/batches` не смог подтвердить pricing authority), склад
остаётся доступным, а ответ дополняется диагностикой `issuanceAuthority.reason` — безопасный
машинный код и человекочитаемое описание без внутренностей: код pricing-ошибки
(`pricing_authority_missing`, drift каталога/switches) означает, что authority нужно чинить в
коммерции, `engine_unavailable` — что недоступен движок или невалиден `ENGINE_CONTROL_KEY`,
`authority_check_failed` — прочее (смотреть серверный лог). Причина показывается в форме выпуска
рядом с заблокированной кнопкой и одновременно пишется в серверный лог
(`openkeys issuance authority check failed`).
