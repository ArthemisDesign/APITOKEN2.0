...
- Mobile chrome (viewport ≤1023px, including iPhone 14 Pro Max portrait 430×932 and landscape 932×430): a sticky top bar (`--admin-chrome`, 56px plus safe-area) keeps brand, realtime dot, language, refresh and theme reachable. The 18 admin sections open from a burger into a full-height grouped drawer (`#admin-nav`); the drawer closes on route change, Escape, or a second burger tap, and `main` is `inert` while it is open. Error Center docks to the bottom of the viewport and hides while the drawer is open, so a source failure cannot cover the chrome or the page title. Every `TableCard` table stacks into a labeled card per row (`data-label` copied from `thead`); wide min-width tables are not panned sideways. Toolbars become a single column, partner subnav stays a 44px chip scroller, and drawer links use a 44px tap floor; chrome icon buttons stay 40px so `apiToken.sale` still fits on a 375px phone. The viewport meta sets `viewport-fit=cover` so safe-area insets reach the chrome, toasts and Error Center. Desktop (≥1024px) keeps the existing 238px sticky sidebar and table layout.
