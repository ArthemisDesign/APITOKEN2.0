...
- [x] `master` admission requires host-owned `promotion/eligible` for the exact identity, or a
      live hotfix attestation.
- [x] Direct push to `master` without attestation does not deploy and raises quarantine/audit.
- [x] `AGENTS.md`, `BRANCHES.md`, `CONTRIBUTING.md`, `docs/ops/DEPLOYMENT.md` describe the
...
### 2026-08-23 — Phase 7 fail-closed production admission
SHA: *(this commit; exact SHA recorded after merge)*   watchdog: pending
Result: Add a root-owned production admission check for the exact master SHA/tree. After a one-cycle
bootstrap, it accepts only unexpired `promotion-eligible.json` or `hotfix-eligible.json` records with
Unix identity `deploy`, trusted policy and record digests, and matching mode. Rejection records
`admission-rejected.sha` and fails before candidate tests/migrations/deploy. Add a 15-second emergency
guard that stops only `staging.slice` when `MemAvailable < 12G` or any production readiness endpoint
is red; hotfix admission never depends on stage state. Update all contributor/runbook contracts to the
mandatory exact-SHA stage→attest→master flow.
Checks actually run: `bash deploy/staging-phase7.test.sh`; `bash deploy/staging-phase6-drills.test.sh`;
`bash deploy/staging-foundation.test.sh`; `bash -n deploy/*.sh`;
`./deploy/host-image-gate.sh`; `git diff --check`.
Deviation from this plan / from STAGING_ENVIRONMENT.md: this enabling SHA uses a one-cycle bootstrap
marker so it can install the admission code without self-requiring a record generated by code that did
not exist. The watchdog writes `staging-admission.enabled` only after this complete GREEN cycle; every
later SHA is fail-closed.
Next: merge this bootstrap on GREEN, prove the enable marker and unattended rejection contract, then
close Phase 7. Do not start Phase 8.
