...
Started: 13:44 · <a href="{commit url}">commit</a> · <a href="{deployment url}">deployments</a>
```

Каждый фазовый статус — правка этого сообщения (счётчик правок не ограничен). Финал: все
✅ + длительность. Карантин: отдельное сообщение в 🚨 Critical + правка деплой-сообщения на
❌ с указанием упавшей фазы и первых ~500 символов причины (`wd_die`-строка из journald,
этап 3; до этапа 3 — только фаза и ссылка на статус).
