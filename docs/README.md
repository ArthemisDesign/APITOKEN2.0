...
- [STAGING_ENVIRONMENT.md](ops/STAGING_ENVIRONMENT.md) — staging implementation plan (v8, owner-approved 2026-08-22, not implemented): architecture, invariants, inventory, locked decisions. Pair: [STAGING_AGENT_PLAN.md](ops/STAGING_AGENT_PLAN.md). Kickoff: [STAGING_AGENT_PROMPT.md](ops/STAGING_AGENT_PROMPT.md).
- [STAGING_AGENT_PLAN.md](ops/STAGING_AGENT_PLAN.md) — binding execution plan for the staging twin. Another agent must follow this file and update it in the same commit as the work (status board, checklists, SHA journal). First code remains the production `contour-config` extract. Architecture stays in STAGING_ENVIRONMENT.
- [STAGING_AGENT_PROMPT.md](ops/STAGING_AGENT_PROMPT.md) — paste-ready kickoff for a new executing session. The agent creates a `/goal` first, then follows STAGING_AGENT_PLAN.
