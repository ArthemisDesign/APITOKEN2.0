...
Два периода в каждом календарном месяце. Всё считается в **UTC** (детерминированно; часовой
пояс можно вынести в конфиг позже).

| Период | Границы | Дни |
|---|---|---|
| P1 | `[1-е 00:00, 16-е 00:00)` | 1–15 |
| P2 | `[16-е 00:00, 1-е следующего месяца 00:00)` | 16–конец месяца |

Ключ периода — `YYYY-MM-P1` / `YYYY-MM-P2` (например `2026-07-P1`). Граница конца периода —
**эксклюзивная** и равна началу следующего (00:00 UTC 16-го или 1-го). Декабрь P2 → январь P1
следующего года обрабатывается корректно.
